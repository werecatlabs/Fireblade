...
*/
static DIR *win32_opendir( const char *szPath )
{
    DIR *nd;
    ...
    nd = (DIR *)calloc(
        1, sizeof( DIR ) + strlen( szPath ) + strlen( win32_SLASH ) + strlen( win32_SUFFIX ) );
...
*/
static struct dirent *win32_readdir( DIR *dirp )
{
    errno = 0;
...
*/
static int win32_closedir( DIR *dirp )
{
    int rc;
...
*/
static void win32_rewinddir( DIR *dirp )
{
    errno = 0;
...
*/
static long win32_telldir( DIR *dirp )
{
    errno = 0;
...
/*
  ...
  any such system.
*/
static void win32_seekdir( DIR *dirp, long lPos )
{
    errno = 0;
