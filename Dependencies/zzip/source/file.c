...
zzip_ssize_t zzip_file_read( ZZIP_FILE *fp, void *buf, zzip_size_t len )
{
    ...
    if( fp->method ) /* method != 0   == 8, inflate */
    {
        ...
        do
        {
            ...
            {
                zzip_size_t cl = ( fp->crestlen < ZZIP_32K ? fp->crestlen : ZZIP_32K );
                /*  zzip_size_t cl =
...
#    define O_NONBLOCK 0
    ...
    {
        ZZIP_FILE *fp = zzip_open_shared_io( stream, filename, o_flags, o_modes, 0, 0 );
...
ZZIP_FILE *zzip_open( zzip_char_t *filename, int o_flags )
{
    ...
    {
        o_flags ^= ZZIP_CASEINSENSITIVE;
        o_modes |= ZZIP_CASELESS;
    }
    if( o_flags & ZZIP_IGNOREPATH )
    {
        o_flags ^= ZZIP_IGNOREPATH;
        o_modes |= ZZIP_NOPATHS;
    }
 ...
 * it). On error this function will return null setting => errno(3).
 */
ZZIP_FILE *zzip_open_shared_io( ZZIP_FILE *stream, zzip_char_t *filename, int o_flags, int o_modes,
...
try_real:
    ...
    {
        zzip_plugin_io_t os = ( o_modes & ZZIP_ALLOWREAL ) ? zzip_get_default_io() : io;
        ...
        while( ( p = strrchr( basename, '/' ) ) )
        {
            ...
            *p = '\0';
            ...
            else
            {
                ...
                {
#ifdef _MSC_VER
                    dir->realname = _strdup( basename );
#else
                    dir->realname = strdup( basename );
#endif
                }
            }
...
ZZIP_FILE *zzip_open_shared_io( ZZIP_FILE *stream, zzip_char_t *name, int o_flags, int o_modes,
                                zzip_strings_t *ext, zzip_plugin_io_t io );
ZZIP_FILE *zzip_open_ext_io( zzip_char_t *name, int o_flags, int o_modes, zzip_strings_t *ext,
                             zzip_plugin_io_t io );
ZZIP_DIR *zzip_opendir_ext_io( zzip_char_t *name, int o_modes, zzip_strings_t *ext,
                               zzip_plugin_io_t io );
...
ZZIP_DIR *zzip_opendir_ext_io( zzip_char_t *name, int o_modes, zzip_strings_t *ext, zzip_plugin_io_t io )
{
    if( !io )
