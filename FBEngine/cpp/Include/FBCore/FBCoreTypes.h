...
namespace fb
{
    ...
    /// Used represent a boolean.
    using bool32 = int;

    /// Used represent a 32 bit hash value.
    using hash32 = u32;

    /// Used represent a 64 bit hash value.
    using hash64 = s64;
...
#    if defined( FB_PLATFORM_WIN32 ) && defined( _MSC_VER )
#        define FB_ASSERT_TRUE( _CONDITION_ ) \
            if( ( _CONDITION_ ) ) \
            { \
                __debugbreak(); \
            }
#        define FB_ASSERT( _CONDITION_ ) \
            if( !( _CONDITION_ ) ) \
            { \
                __debugbreak(); \
            }
#    else
#        include <assert.h>
#        define FB_ASSERT_TRUE( _CONDITION_ ) assert( ( _CONDITION_ ) ? false : true );
#        define FB_ASSERT( _CONDITION_ ) assert( _CONDITION_ );
#    endif
#else
#    define FB_ASSERT_TRUE( _CONDITION_ )
#    define FB_ASSERT( _CONDITION_ )
...
#ifdef FB_PLATFORM_WIN32
#    define FB_DEPRECATED( x ) __declspec( deprecated ) x
#else
#    define FB_DEPRECATED( x ) x
#endif

#ifdef FB_PLATFORM_WIN32
#    define FB_UNUSED( unused ) (void)( ( unused ) )
#else
#    define FB_UNUSED( unused ) (void)( ( unused ) )
#endif

#ifdef FB_PLATFORM_LINUX
#    include <cmath>
using std::isfinite;
#endif
