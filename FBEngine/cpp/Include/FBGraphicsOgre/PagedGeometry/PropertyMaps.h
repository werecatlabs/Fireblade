...
namespace Forests
{
    ...
    public:
        ...
        Ogre::Real _getDensityAt_Unfiltered( Ogre::Real x, Ogre::Real z,
                                             const Ogre::RealRect &mapBounds );
    ...
    public:
        ...
        {
            return filter == MAPFILTER_NONE ? _getColorAt( x, z, mapBounds )
                                            : _getColorAt_Bilinear( x, z, mapBounds );
        }

        /** \brief Gets the color value at the specified position

        The unpacks the 32-bit color value into an Ogre::ColourValue and returns it. */
        Ogre::ColourValue getColorAt_Unpacked( Ogre::Real x, Ogre::Real z,
                                               const Ogre::RealRect &mapBounds )
        {
            Ogre::uint32 c = filter == MAPFILTER_NONE ? _getColorAt( x, z, mapBounds )
                                                      : _getColorAt_Bilinear( x, z, mapBounds );
    ...
    private:
        ...
        Ogre::uint32 _interpolateColor( Ogre::uint32 color1, Ogre::uint32 color2, Ogre::Real ratio,
                                        Ogre::Real ratioInv );
