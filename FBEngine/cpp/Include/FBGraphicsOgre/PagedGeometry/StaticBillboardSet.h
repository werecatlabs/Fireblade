...
namespace Forests
{
    ...
    class StaticBillboardSet
    {
        ...
        public:
            ...
            StaticBillboard( const Ogre::Vector3 &pos, float xScale, float yScale,
                             const Ogre::ColourValue &clr, Ogre::uint16 texcrdIndexU,
                             Ogre::uint16 texcrdIndexV ) :
                xPos( (float)pos.x ),
                yPos( (float)pos.y ),
                zPos( (float)pos.z ),
    ...
    public:
        ...
        */
        void createBillboard( const Ogre::Vector3 &position, float xScale = 1.0f, float yScale = 1.0f,
                              ...
                              Ogre::uint16 texcoordIndexU = 0, Ogre::uint16 texcoordIndexV = 0 )
        {
            if( mRenderMethod == BB_METHOD_ACCELERATED )
            {
                mBillboardBuffer.push_back( new StaticBillboard( position, xScale, yScale, color,
    ...
    private:
        ...
        Ogre::MaterialPtr getFadeMaterial( const Ogre::MaterialPtr &protoMaterial,
                                           Ogre::Real visibleDist, Ogre::Real invisibleDist );
