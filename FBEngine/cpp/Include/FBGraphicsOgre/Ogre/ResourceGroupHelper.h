...
namespace fb
{
    ...
    private:
        ...
        void visitRecursivelyRenderablesFrom( Ogre::OverlayContainer *pOverlayContainer,
                                              Ogre::Renderable::Visitor &pVisitor,
                                              bool debugRenderable = false );

        ///\brief a map [key : nameOfTheRessourceGroup]/[value : last modification time on the hdd from
        ///the files of the ResourceGroup]
        std::map<std::string, time_t> mRessourceGroupModificationTimes;

        // ------ helper classes

        /// \brief this visitor will be used to set the material on known renderable that allow this
        /// operation. for other user class renderable, must be tweaked/changed
        class UpdateMaterialRenderableVisitor : public Ogre::Renderable::Visitor
        {
        private:
            UpdateMaterialRenderableVisitor(
                const UpdateMaterialRenderableVisitor & );  ///<\brief anti copyconstructor
            UpdateMaterialRenderableVisitor &operator=(
                const UpdateMaterialRenderableVisitor & );  ///<\brief anti affector
        public:
            /// \brief default constructor
            UpdateMaterialRenderableVisitor();
            /// \brief called for each renderable
            virtual void visit( Ogre::Renderable *rend, Ogre::ushort lodIndex, bool isDebug,
                                Ogre::Any *pAny = 0 );
        };
        ...
        private:
            ResourceGroupHelperLogListener(
                const ResourceGroupHelperLogListener & );  ///<\brief anti copyconstructor
            ResourceGroupHelperLogListener &operator=(
                const ResourceGroupHelperLogListener & );  ///<\brief anti affector
        ...
        public:
            ...
            virtual void messageLogged( const Ogre::String &message, Ogre::LogMessageLevel lml,
                                        bool maskDebug, const Ogre::String &logName, bool &skipMessage );
    ...
    public:
        ...
        /// \warning there is no garanty that the path order will be the same than during the first
        /// loading.
        Array<ArchivePathAndType> getAllPathAndTypesNames( const std::string &pResourceGroupName );

        /// \brief tries to suppress a Resourcegroup and reload it completely using the provided ordered
        /// locations. \param the name of the resourcegroup \param the path and types of the archives to
        /// be loaded, and in the order to be loaded it will not work correctly if some elements of the
        /// Resourcegroup are still used. \return true if the Resourcegroup was found and correctly
        /// destroy, false otherwise.
        bool reloadAResourceGroup( const std::string &pResourceGroupName,
                                   const Array<ArchivePathAndType> &pLocationToAdd );

        /// \brief this serves the same purpose than reloadAResourceGroup, but it does not
        /// destroy/recreate the ResourceGroup. \param the name of the resourcegroup on the one hand, you
        /// don't need to worry about the path and order of the locations. on the other hand, there is no
        /// test that the resources were really cleared and reloaded. personnally I prefer this thought,
        /// because it's much smarter & less costly in the end.
        ...
        bool checkTimeAndReloadIfNeeded( const std::string &pResourceGroupName,
                                         std::string &pOutLoggedMessages, bool useLog = true );
    };
