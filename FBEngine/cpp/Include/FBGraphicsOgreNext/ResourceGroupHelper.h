...
private:
    ...
    // Ogre::Renderable::Visitor& pVisitor, bool debugRenderable = false);

    ///\brief a map [key : nameOfTheRessourceGroup]/[value : last modification time on the hdd from the
    /// files of the ResourceGroup]
...
public:
    ...
    /// locations. \param the name of the resourcegroup \param the path and types of the archives to be
    /// loaded, and in the order to be loaded it will not work correctly if some elements of the
    /// Resourcegroup are still used. \return true if the Resourcegroup was found and correctly destroy,
    /// false otherwise.
    bool reloadAResourceGroup( const std::string &pResourceGroupName,
                               const std::vector<ArchivePathAndType> &pLocationToAdd );

    /// \brief this serves the same purpose than reloadAResourceGroup, but it does not destroy/recreate
    /// the ResourceGroup. \param the name of the resourcegroup on the one hand, you don't need to worry
    /// about the path and order of the locations. on the other hand, there is no test that the resources
    /// were really cleared and reloaded. personnally I prefer this thought, because it's much smarter &
    /// less costly in the end.
