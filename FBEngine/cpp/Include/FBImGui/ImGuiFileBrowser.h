...
namespace fb
{
    namespace ui
    {
        class ImGuiFileBrowser : public CImGuiElement<IUIFileBrowser>
        {
        public:
            ImGuiFileBrowser();
            ~ImGuiFileBrowser() override;

            /** @copydoc ISharedObject::update */
            void update() override;

            /** @copydoc IUIFileBrowser::show */
            bool show() override;
            ...
            */
            ...
            /** @copydoc IUIFileBrowser::getFileExtension */
            String getFileExtension() const override;

            /** @copydoc IUIFileBrowser::setFileExtension */
             ...
             * the size, a DialogMode enum value defining in which mode the dialog should operate and
             * optionally the extensions that are valid for opening. Note that the select directory mode
             * doesn't need any extensions.
             */
            bool showFileDialog( const std::string &label, DialogMode mode,
                                 const ImVec2 &sz_xy = ImVec2( 0, 0 ),
                                 const std::string &valid_types = "*.*" );

            /* Store the opened/saved file name or dir name (incase of selectDirectoryDialog) and the
             * absolute path to the selection Should only be accessed when above functions return true
             * else may contain garbage.
             */
            std::string selected_fn;
            std::string selected_path;
            std::string extension; // Store the saved file extension
        ...
        private:
            ...
            void initCurrentPath();  // Helper function for UNIX based system to load Absolute path using
                                     // realpath
#endif
            ...
            bool show_hidden, show_inputbar_combobox, is_dir, is_appearing, filter_dirty, validate_file,
                 show_all_valid_files;
            char input_fn[256];

            Array<std::string> valid_exts;
            Array<std::string> current_dirlist;
            Array<Info> subdirs;
            Array<Info> subfiles;
            ...
            Array<const Info *> filtered_dirs; // Note: We don't need to call delete. It's just
            // for storing filtered items from subdirs and
            // subfiles so we don't use PassFilter every frame.
            Array<const Info *> filtered_files;
            Array<std::reference_wrapper<std::string>> inputcb_filter_files;
