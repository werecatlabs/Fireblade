...
namespace Ogre
{
    ...
    bool TerrainTransform::preAddToRenderState( const RenderState *renderState, Pass *srcPass,
                                                Pass *dstPass )
    {
    ...
    const Ogre::String &TerrainTransform::getType() const
    {
        return Type;
    }

    int TerrainTransform::getExecutionOrder() const
    {
        return RTShader::FFP_TRANSFORM;
    }

    void TerrainTransform::copyFrom( const SubRenderState &rhs )
    {
    }
    ...
    bool TerrainTransform::createCpuSubPrograms( ProgramSet *programSet )
    {
        ...
        auto lodMorph = vsProgram->resolveAutoParameterInt( GpuProgramParameters::ACT_CUSTOM, GCT_FLOAT2,
                                                            Terrain::LOD_MORPH_CUSTOM_PARAM );

        auto positionIn =
            vsEntry->resolveInputParameter( Parameter::SPC_POSITION_OBJECT_SPACE, posType );
        ...
        if( mCompressed )
        {
            ...
            auto height =
                vsEntry->resolveInputParameter( Parameter::SPC_TEXTURE_COORDINATE0, GCT_FLOAT1 );
            ...
            stage.callFunction( "expandVertex",
                                { In( mPointTrans ), In( mBaseUVScale ), In( positionIn ), In( height ),
                                  Out( position ), Out( uv ) } );
            positionIn = position;
        }

        stage.callFunction( "FFP_Transform", wvpMatrix, positionIn, positionOut );
        stage.callFunction( "applyLODMorph", { In( delta ), In( lodMorph ),
                                               InOut( positionOut ).mask( heightAxis[mAlign] ) } );
