    ...
    namespace render
    {
        ...
        void HDRListener::notifyCompositor( Ogre::CompositorInstance *instance )
        {
            ...
            while( defIter.hasMoreElements() )
            {
                ...
                if( def->name == "rt_bloom0" )
                {
                    ...
                    mBloomTexWeights[0][0] = mBloomTexWeights[0][1] = mBloomTexWeights[0][2] =
                        Ogre::Math::gaussianDistribution( 0, 0, deviation );
                    mBloomTexWeights[0][3] = 1.0f;

                    // 'pre' samples
                    for( int i = 1; i < 8; ++i )
                    {
                        mBloomTexWeights[i][0] = mBloomTexWeights[i][1] = mBloomTexWeights[i][2] =
                            1.25f * Ogre::Math::gaussianDistribution( i, 0, deviation );
        ...
        void HDRListener::notifyMaterialSetup( Ogre::uint32 pass_id, Ogre::MaterialPtr &mat )
        {
            ...
            case 701:  // rt_bloom1
            {
                ...
                const Ogre::String &progName =
                    mat->getBestTechnique()->getPass( 0 )->getFragmentProgramName();
