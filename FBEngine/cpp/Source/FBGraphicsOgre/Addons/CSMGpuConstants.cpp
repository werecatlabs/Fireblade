...
namespace fb
{
    ...
    {
        mParamsScaleBias = Ogre::GpuProgramManager::getSingletonPtr()->createSharedParameters(
            "params_shadowMatrixScaleBias" );
        for( size_t i = 1; i < cascadeCount; i++ )
        {
            mParamsScaleBias->addConstantDefinition(
                "texMatrixScaleBias" + Ogre::StringConverter::toString( i ), Ogre::GCT_FLOAT4 );
        }

        mParamsShadowMatrix =
            Ogre::GpuProgramManager::getSingletonPtr()->createSharedParameters( "params_shadowMatrix" );
    ...
    void CSMGpuConstants::updateCascade( const Ogre::Camera &texCam, size_t index )
    {
        if( index == 0 )
        {
            ...
            Ogre::Matrix4 texMatrix0 = PROJECTIONCLIPSPACE2DTOIMAGESPACE_PERSPECTIVE *
                                       texCam.getProjectionMatrixWithRSDepth() * mFirstCascadeViewMatrix;
            mParamsShadowMatrix->setNamedConstant( "texMatrix0", texMatrix0 );
        }
        else
        {
            hack = PROJECTIONCLIPSPACE2DTOIMAGESPACE_PERSPECTIVE *
                   texCam.getProjectionMatrixWithRSDepth() * texCam.getViewMatrix();
            ...
            Ogre::Real offCenter = width1 / ( 2.0f * width0 ) - 0.5;

            Ogre::RenderSystem *rs = Ogre::Root::getSingletonPtr()->getRenderSystem();
            float depthRange =
                Ogre::Math::Abs( rs->getMinimumDepthInputValue() - rs->getMaximumDepthInputValue() );
            ...
            mParamsScaleBias->setNamedConstant(
                "texMatrixScaleBias" + Ogre::StringConverter::toString( index ), result );
        }
    }
