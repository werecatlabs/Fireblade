...
namespace fb
{
    namespace render
    {
        ...
        BasicSceneManager::BasicSceneManager( const Ogre::String &name ) : Ogre::SceneManager( name )
        {
        ...
        BasicSceneManager::~BasicSceneManager()
        {
            ...
            mSceneRoot->~SceneNode();
            mSceneRoot = nullptr;
        }

        const Ogre::String &BasicSceneManager::getTypeName( void ) const
        {
            static Ogre::String type = "BasicSceneManager";
            return type;
        }

        bool BasicSceneManager::frameRenderingQueued( const Ogre::FrameEvent &evt )
        {
            getRootSceneNode()->_update( true, false );
            return true;
        }

        void BasicSceneManager::_updateSceneGraph( Ogre::Camera *camera )
        {
        ...
        void BasicSceneManager::_findVisibleObjects( Ogre::Camera *cam,
                                                     Ogre::VisibleObjectsBoundsInfo *visibleBounds,
                                                     bool onlyShadowCasters )
        {
            addSceneNodes( getRootSceneNode(), cam, onlyShadowCasters, visibleBounds );
        }

        void BasicSceneManager::_addVisibleObjects( Ogre::SceneNode *sceneNode, Ogre::Camera *cam,
                                                    bool onlyShadowCasters,
                                                    Ogre::VisibleObjectsBoundsInfo *visibleBounds )
        {
            ...
            while( it.hasMoreElements() )
            {
                ...
                getRenderQueue()->processVisibleObject( mo, cam, onlyShadowCasters, visibleBounds );
            }
        }

        void BasicSceneManager::addSceneNodes( Ogre::SceneNode *sceneNode, Ogre::Camera *cam,
                                               bool onlyShadowCasters,
                                               Ogre::VisibleObjectsBoundsInfo *visibleBounds )
        {
            ...
            while( childIt.hasMoreElements() )
            {
                ...
                _addVisibleObjects( child, cam, onlyShadowCasters, visibleBounds );

                addSceneNodes( child, cam, onlyShadowCasters, visibleBounds );
            }
        }

        void BasicSceneManager::_setActiveCompositorChain( Ogre::CompositorChain *chain )
        {
            mActiveCompositorChain = chain;
        }

        
        const Ogre::Pass *BasicSceneManager::_setPass( const Ogre::Pass *pass, bool evenIfSuppressed,
                                                       bool shadowDerivation )
        {
            ...
            //	else
            //	{
            //		if(lateTech && lateTech->getNumPasses() > 0)
            ...
            //	{
            //		// Unbind program?
            //		if (mDestRenderSystem->isGpuProgramBound(Ogre::GPT_HULL_PROGRAM))
            //		{
            //			mDestRenderSystem->unbindGpuProgram(Ogre::GPT_HULL_PROGRAM);
            //		}
            ...
            //	{
            //		// Unbind program?
            //		if (mDestRenderSystem->isGpuProgramBound(Ogre::GPT_DOMAIN_PROGRAM))
            //		{
            //			mDestRenderSystem->unbindGpuProgram(Ogre::GPT_DOMAIN_PROGRAM);
            //		}
            ...
            //			pass->hasSeparateSceneBlendingOperations() ? pass->getSceneBlendingOperation() :
            ...
            //		{
            //			bool bSearch = false;
            //			Ogre::String referencedCompositorName;
            //			Ogre::String referencedTextureName;
            ...
            //			}
            //			Ogre::CompositorInstance* refComp =
            //currentChain->getCompositor(pTex->getReferencedCompositorName()); 			if (refComp == 0)
            //			{
            //				referencedCompositorName = pTex->getReferencedCompositorName();
            //				bSearch = true;
            //			}
            //			else
            //			{
            ...
            //				if (refTex.isNull())
            //				{
            //					referencedTextureName = pTex->getReferencedTextureName();
            //					bSearch = true;
            //				}
            //				else
            //				{
            ...
            //			if(bSearch)
            //			{
            //				Ogre::MaterialPtr enemyDistortionMaterial =
            //Ogre::MaterialManager::getSingletonPtr()->getByName("EnemyDistortion");
            ...
            //				Array<SmartPtr<IViewport>> viewports = window->getViewports();
            //				for(u32 i=0; i<viewports.size(); ++i)
            //				{
            //					SmartPtr<IViewport>& viewport = viewports[i];

            //					Ogre::Viewport* vp = nullptr;
            //					viewport->_getObject((void**)&vp);

            //					Ogre::CompositorManager* compositorManager =
            //Ogre::CompositorManager::getSingletonPtr(); 					Ogre::CompositorChain* currentChain =
            //compositorManager->getCompositorChain(vp); 					if (!currentChain)
            //					{
            //						continue;
            //					}
            ...
            //					if(!refTex.isNull())
            //					{
            //						std::cout << "ZOrder: " << vp->getZOrder() << " Texture: " <<
            //referencedTextureName << " Reference: " << referencedCompositorName << std::endl;
            ...
            //		&& mIlluminationStage == Ogre::SceneManager::IRS_RENDER_TO_TEXTURE
        ...
        Ogre::SceneNode *BasicSceneManager::createSceneNodeImpl( void )
        {
            return new Ogre::SceneNode( this );
        }

        Ogre::SceneNode *BasicSceneManager::createSceneNodeImpl( const Ogre::String &name )
        {
            return new Ogre::SceneNode( this, name );
        }

        void BasicSceneManager::destroySceneNode( const Ogre::String &name )
        {
            Ogre::LogManager::getSingletonPtr()->logMessage( Ogre::String( "Node destroyed: " ) + name );
            ...
            {
                OGRE_EXCEPT( Ogre::Exception::ERR_ITEM_NOT_FOUND, "SceneNode '" + name + "' not found.",
                             "SceneManager::destroySceneNode" );
            }

            Ogre::SceneNode *sceneNode;  // = i->second;
            ...
            for( ai = mAutoTrackingSceneNodes.begin(); ai != aiend; )
            {
                ...
                SceneNode *n = *curri;
                // Tracking this node
                if( n->getAutoTrackTarget() == sceneNode )
                {
            ...
            // behaves differently)
            Ogre::Node *parentNode = sceneNode->getParent();
            if( parentNode )
            {
        ...
        Ogre::Entity *BasicSceneManager::createEntity( const Ogre::String &entityName,
                                                       Ogre::SceneManager::PrefabType ptype )
        {
            ...
            OGRE_EXCEPT( Ogre::Exception::ERR_ITEM_NOT_FOUND,
                         "Unknown prefab type for entity " + entityName, "SceneManager::createEntity" );
        }

        
        Ogre::Entity *BasicSceneManager::createEntity( Ogre::SceneManager::PrefabType ptype )
        {
            Ogre::String name = mMovableNameGenerator.generate();
            return createEntity( name, ptype );
        }

        
        Ogre::Entity *BasicSceneManager::createEntity(
            const Ogre::String &entityName, const Ogre::String &meshName,
            const Ogre::String &groupName /* = ResourceGroupManager::AUTODETECT_RESOURCE_GROUP_NAME */ )
        {
            // delegate to factory implementation
            Ogre::NameValuePairList params;
            params["mesh"] = meshName;
            params["resourceGroup"] = groupName;
            return static_cast<Ogre::Entity *>(
                createMovableObject( entityName, Ogre::EntityFactory::FACTORY_TYPE_NAME, &params ) );
        }
        
        Ogre::Entity *BasicSceneManager::createEntity( const Ogre::String &entityName,
                                                       const Ogre::MeshPtr &pMesh )
        {
            return createEntity( entityName, pMesh->getName(), pMesh->getGroup() );
        }
        
        Ogre::Entity *BasicSceneManager::createEntity( const Ogre::String &meshName )
        {
            Ogre::String name = mMovableNameGenerator.generate();
            // note, we can't allow groupName to be passes, it would be ambiguous (2 string params)
            return createEntity( name, meshName,
                                 Ogre::ResourceGroupManager::AUTODETECT_RESOURCE_GROUP_NAME );
        }
        
        Ogre::Entity *BasicSceneManager::createEntity( const Ogre::MeshPtr &pMesh )
        {
            Ogre::String name = mMovableNameGenerator.generate();
            return createEntity( name, pMesh );
        }

        void BasicSceneManager::clearScene( void )
        {
        ...
        void BasicSceneManager::renderSingleObject( Ogre::Renderable *rend, const Ogre::Pass *pass,
        ...
        const Ogre::String BasicSceneManagerFactory::FACTORY_TYPE_NAME = "BasicSceneManager";
        
        void BasicSceneManagerFactory::initMetaData( void ) const
        {
        ...
        Ogre::SceneManager *BasicSceneManagerFactory::createInstance( const Ogre::String &instanceName )
        {
            return OGRE_NEW BasicSceneManager( instanceName );
        }
        
        void BasicSceneManagerFactory::destroyInstance( Ogre::SceneManager *instance )
        {
