...
namespace fb
{
                                                                      ...
                                                                      bool isDebug, Ogre::Any *pAny )
    {
        ...
        if( !mat.isNull() )
        {
            ...
            // unfortunately, the renderable gives access only to a const SmartPtr<IMaterial>.
    ...
    void ResourceGroupHelper::ResourceGroupHelperLogListener::messageLogged( const Ogre::String &message,
                                                                             ...
                                                                             bool &skipMessage )
    {
        ...
        bool lErrorfound = Ogre::StringUtil::match( message, pattern1, true ) ||
                           Ogre::StringUtil::match( message, pattern2, true );
        if( lErrorfound )
        {
    ...
    ResourceGroupHelper::ResourceGroupHelper( void ) : mRessourceGroupModificationTimes()
    {
        ...
        mRessourceGroupModificationTimes[Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME] =
            veryBig;
    }
    ...
    Array<ResourceGroupHelper::ArchivePathAndType> ResourceGroupHelper::getAllPathAndTypesNames(
        const std::string &pResourceGroupName )
    {
        ...
        Ogre::FileInfoListPtr fli_dirs =
            Ogre::ResourceGroupManager::getSingleton().listResourceFileInfo( pResourceGroupName, true );

        Array<ResourceGroupHelper::ArchivePathAndType> lDirectoryInfos;
    ...
    bool ResourceGroupHelper::reloadAResourceGroup(
        const std::string &pResourceGroupName,
        const Array<ResourceGroupHelper::ArchivePathAndType> &pLocationToAdd )
    {
        ...
        Array<ResourceGroupHelper::ArchivePathAndType>::const_iterator iterLoc = pLocationToAdd.begin();
        Array<ResourceGroupHelper::ArchivePathAndType>::const_iterator iterLocEnd = pLocationToAdd.end();
        for( ; iterLoc != iterLocEnd; iterLoc++ )
        {
            // add the location in the resourceGroup
            resGroupMgr.addResourceLocation( iterLoc->first, iterLoc->second, pResourceGroupName,
                                             false );
        }
    ...
    void ResourceGroupHelper::updateOnEveryRenderable()
    {
        ...
        Array<std::string> allAvailableTypes;
        Ogre::Root::MovableObjectFactoryIterator iterFactory =
        ...
        Ogre::SceneManagerEnumerator::SceneManagerIterator iterSceneManager =
        ...
        for( ; iterSceneManager.hasMoreElements(); )
        {
            ...
            Array<std::string>::iterator iterMovableType = allAvailableTypes.begin();
            Array<std::string>::iterator iterMovableTypeEnd = allAvailableTypes.end();
            for( ; iterMovableType != iterMovableTypeEnd; iterMovableType++ )
            {
                Ogre::SceneManager::MovableObjectIterator iterMovable =
                    scMgr->getMovableObjectIterator( *iterMovableType );
        ...
        // 3 / visit overlays!
        {
            // Ogre::OverlayManager::OverlayMapIterator iterOverlay =
            // Ogre::OverlayManager::getSingleton().getOverlayIterator();
            ...
            //    Ogre::Overlay::Overlay2DElementsIterator iterOverlayElem =
            //    lOverlay->get2DElementsIterator(); for(;iterOverlayElem.hasMoreElements();)
            //    {
    ...
    void ResourceGroupHelper::visitRecursivelyRenderablesFrom( Ogre::OverlayContainer *pOverlayContainer,
                                                               Ogre::Renderable::Visitor &pVisitor,
                                                               bool debugRenderable )
    {
        // call on 'this'
        ...
        {
            Ogre::OverlayContainer::ChildContainerIterator childContainerIter =
                pOverlayContainer->getChildContainerIterator();
        ...
        if( iterInfoTime != mRessourceGroupModificationTimes.end() )
        {
            if( iterInfoTime->second < lastModificationTime )
            {
                ...
                Ogre::LogManager::getSingleton().logMessage(
                    "time has evoluated ! reload the resourcegroup!" );
                // use log if needed
                if( useLog )
                {
