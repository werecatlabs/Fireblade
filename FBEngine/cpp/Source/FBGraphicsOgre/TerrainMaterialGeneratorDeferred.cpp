...
#include "TerrainMaterialGeneratorDeferred.h"
...
namespace Ogre
{
    ...
    TerrainMaterialGeneratorDeferred::TerrainMaterialGeneratorDeferred()
    {
        // define the layers
    ...
    TerrainMaterialGeneratorDeferred::SM2Profile::SM2Profile( TerrainMaterialGenerator *parent,
    ...
    TerrainMaterialGeneratorDeferred::SM2Profile::~SM2Profile()
    {
        OGRE_DELETE mShaderGen;
    }

    void TerrainMaterialGeneratorDeferred::SM2Profile::requestOptions( Terrain *terrain )
    {
    ...
    bool TerrainMaterialGeneratorDeferred::SM2Profile::isVertexCompressionSupported() const
    {
        return true;
    }

    bool TerrainMaterialGeneratorDeferred::SM2Profile::isGlobalColourMapEnabled() const
    {
        return mGlobalColourMapEnabled;
    }

    void TerrainMaterialGeneratorDeferred::SM2Profile::setGlobalColourMapEnabled( bool enabled )
    {
    ...
    bool TerrainMaterialGeneratorDeferred::SM2Profile::isLightmapEnabled() const
    {
        return mLightmapEnabled;
    }

    void TerrainMaterialGeneratorDeferred::SM2Profile::setLightmapEnabled( bool enabled )
    {
    ...
    bool TerrainMaterialGeneratorDeferred::SM2Profile::isCompositeMapEnabled() const
    {
        return mCompositeMapEnabled;
    }

    void TerrainMaterialGeneratorDeferred::SM2Profile::setCompositeMapEnabled( bool enabled )
    {
    ...
    bool TerrainMaterialGeneratorDeferred::SM2Profile::_isSM3Available() const
    {
        return mSM3Available;
    }

    bool TerrainMaterialGeneratorDeferred::SM2Profile::_isSM4Available() const
    {
        return mSM4Available;
    }

    uint8 TerrainMaterialGeneratorDeferred::SM2Profile::getMaxLayers( const Terrain *terrain ) const
    {
        ...
        return static_cast<uint8>( 32 );
    }

    Ogre::MaterialPtr TerrainMaterialGeneratorDeferred::SM2Profile::generate( const Terrain *terrain )
    {
        // re-use old material if exists
        auto mat = terrain->_getMaterial();
        if( mat.isNull() )
        {
    ...
    Ogre::MaterialPtr TerrainMaterialGeneratorDeferred::SM2Profile::generateForCompositeMap(
        const Terrain *terrain )
    {
        // re-use old material if exists
        auto mat = terrain->_getCompositeMapMaterial();
        if( mat.isNull() )
        {
            MaterialManager &matMgr = MaterialManager::getSingleton();
    ...
    void TerrainMaterialGeneratorDeferred::SM2Profile::addTechnique( const Ogre::MaterialPtr &mat,
                                                                     const Terrain *terrain,
                                                                     TechniqueType tt )
    {
        ...
        if( tt == HIGH_LOD || tt == RENDER_COMPOSITE_MAP )
        {
            ...
            // uint numBlendTextures = terrain->getBlendTextureCount();
    ...
    void TerrainMaterialGeneratorDeferred::SM2Profile::updateParams( const Ogre::MaterialPtr &mat,
                                                                     const Terrain *terrain )
    {
        mShaderGen->updateParams( this, mat, terrain, false );
    }

    void TerrainMaterialGeneratorDeferred::SM2Profile::updateParamsForCompositeMap(
        const Ogre::MaterialPtr &mat, const Terrain *terrain )
    {
        mShaderGen->updateParams( this, mat, terrain, true );
    }

    TerrainMaterialGeneratorDeferred::SM2Profile::ShaderHelper::ShaderHelper()
    {
    }
    ...
    TerrainMaterialGeneratorDeferred::SM2Profile::ShaderHelper::generateVertexProgram(
    ...
    HighLevelGpuProgramPtr
    TerrainMaterialGeneratorDeferred::SM2Profile::ShaderHelper::generateFragmentProgram(
        const SM2Profile *prof, const Terrain *terrain, TechniqueType tt )
    {
    ...
    void TerrainMaterialGeneratorDeferred::SM2Profile::ShaderHelper::generateVertexProgramSource(
        const SM2Profile *prof, const Terrain *terrain, TechniqueType tt,
        StringUtil::StrStreamType &outStream )
    {
        ...
        if( tt == HIGH_LOD )
        {
            std::fstream shaderSrc( "TerrainDeferredVP.cg", std::fstream::out );
            shaderSrc << outStream.str().c_str();
        }
        else if( tt == LOW_LOD )
        {
            std::fstream shaderSrc( "TerrainDeferredLowVP.cg", std::fstream::out );
            shaderSrc << outStream.str().c_str();
        }
    }

    void TerrainMaterialGeneratorDeferred::SM2Profile::ShaderHelper::generateFragmentProgramSource(
        const SM2Profile *prof, const Terrain *terrain, TechniqueType tt,
        StringUtil::StrStreamType &outStream )
    {
        if( tt == HIGH_LOD )
        {
            ...
            auto stream = fileSystem->open( "TerrainDeferredFP.cg" );
            if( stream )
            {
            ...
            else
            {
                ...
                if( tt != LOW_LOD )
                {
                    uint maxLayers = prof->getMaxLayers( terrain );
                    uint numLayers =
                        std::min( maxLayers, static_cast<uint>( terrain->getLayerCount() ) );
        ...
        if( tt == HIGH_LOD )
        {
            std::fstream shaderSrc( "TerrainDeferredHighFP.cg", std::fstream::out );
            shaderSrc << outStream.str().c_str();
        }
        else if( tt == LOW_LOD )
        {
            std::fstream shaderSrc( "TerrainDeferredLowFP.cg", std::fstream::out );
            shaderSrc << outStream.str().c_str();
        }
    }

    void TerrainMaterialGeneratorDeferred::SM2Profile::ShaderHelper::defaultVpParams(
        const SM2Profile *prof, const Terrain *terrain, TechniqueType tt,
    ...
    void TerrainMaterialGeneratorDeferred::SM2Profile::ShaderHelper::defaultFpParams(
        const SM2Profile *prof, const Terrain *terrain, TechniqueType tt,
        const HighLevelGpuProgramPtr &prog )
    {
        GpuProgramParametersSharedPtr params = prog->getDefaultParameters();
        ...
        params->setNamedAutoConstant( "cFarDistance",
                                      Ogre::GpuProgramParameters::ACT_FAR_CLIP_DISTANCE );
        params->setNamedAutoConstant(
            "viewMatrix", GpuProgramParameters::ACT_WORLDVIEW_MATRIX );  // tout sauf Z : VIEW_MATRIX
    }

    void TerrainMaterialGeneratorDeferred::SM2Profile::ShaderHelper::updateParams(
    ...
    void TerrainMaterialGeneratorDeferred::SM2Profile::ShaderHelper::updateVpParams(
        const SM2Profile *prof, const Terrain *terrain, TechniqueType tt,
    ...
    void TerrainMaterialGeneratorDeferred::SM2Profile::ShaderHelper::updateFpParams(
        const SM2Profile *prof, const Terrain *terrain, TechniqueType tt,
        const GpuProgramParametersSharedPtr &params )
    {
        params->setIgnoreMissingParams( true );
    ...
    String TerrainMaterialGeneratorDeferred::SM2Profile::ShaderHelper::getChannel( uint idx )
    {
        uint rem = idx % 4;
        switch( rem )
        {
    ...
    String TerrainMaterialGeneratorDeferred::SM2Profile::ShaderHelper::getVertexProgramName(
        const SM2Profile *prof, const Terrain *terrain, TechniqueType tt )
    {
    ...
    String TerrainMaterialGeneratorDeferred::SM2Profile::ShaderHelper::getFragmentProgramName(
        const SM2Profile *prof, const Terrain *terrain, TechniqueType tt )
    {
    ...
    HighLevelGpuProgramPtr
    TerrainMaterialGeneratorDeferred::SM2Profile::ShaderHelperCg::createVertexProgram(
        const SM2Profile *prof, const Terrain *terrain, TechniqueType tt )
    {
        ...
        ret->setParameter( "profiles", "vs_3_0" );
    ...
    HighLevelGpuProgramPtr
    TerrainMaterialGeneratorDeferred::SM2Profile::ShaderHelperCg::createFragmentProgram(
        const SM2Profile *prof, const Terrain *terrain, TechniqueType tt )
    {
        HighLevelGpuProgramManager &mgr = HighLevelGpuProgramManager::getSingleton();
    ...
    void TerrainMaterialGeneratorDeferred::SM2Profile::ShaderHelperCg::generateVpHeader(
        const SM2Profile *prof, const Terrain *terrain, TechniqueType tt,
        StringUtil::StrStreamType &outStream )
    {
    ...
    void TerrainMaterialGeneratorDeferred::SM2Profile::ShaderHelperCg::generateFpHeader(
        const SM2Profile *prof, const Terrain *terrain, TechniqueType tt,
        StringUtil::StrStreamType &outStream )
    {
        // Main header
        ...
        uint texCoordSet = 1;
        outStream << "float4 uv : TEXCOORD" << texCoordSet++ << ",\n";
        ...
        if( tt != LOW_LOD )
        {
            outStream << "	float3 normal = expand(tex2D(globalNormal, uv.xy));\n";
        }
        ...
        if( tt == LOW_LOD )
        {
            ...
            outStream << "	float4 composite = tex2D(compositeMap, uv.xy);\n"
        ...
        else
        {
            ...
            for( uint i = 0; i < numBlendTextures; ++i )
            {
                outStream << "	float4 blendTexVal" << i << " = tex2D(blendTex" << i << ", uv.xy);\n";
            }
            ...
            // are provided by the "normal" texture, and not in vertex info. We multiply it by the
            // viewMatrix so the normals are in eye-space instead of object-space, as required by
            // deferred shading.
    ...
    void TerrainMaterialGeneratorDeferred::SM2Profile::ShaderHelperCg::generateVpLayer(
    ...
    void TerrainMaterialGeneratorDeferred::SM2Profile::ShaderHelperCg::generateFpLayer(
        const SM2Profile *prof, const Terrain *terrain, TechniqueType tt, uint layer,
        StringUtil::StrStreamType &outStream )
    {
        uint uvIdx = layer / 2;
    ...
    void TerrainMaterialGeneratorDeferred::SM2Profile::ShaderHelperCg::generateVpFooter(
        const SM2Profile *prof, const Terrain *terrain, TechniqueType tt,
        StringUtil::StrStreamType &outStream )
    {
    ...
    void TerrainMaterialGeneratorDeferred::SM2Profile::ShaderHelperCg::generateFpFooter(
        const SM2Profile *prof, const Terrain *terrain, TechniqueType tt,
        StringUtil::StrStreamType &outStream )
    {
        if( terrain->getGlobalColourMapEnabled() && prof->isGlobalColourMapEnabled() )
