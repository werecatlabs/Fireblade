...
namespace Forests
{
    ...
    TreeLoader3D::TreeLoader3D( PagedGeometry *geom, const TBounds &bounds )
    {
        ...
        gridBounds = bounds;
        gridBounds.left =
            pageSize * Math::Floor( ( gridBounds.left - geom->getBounds().left ) / pageSize ) +
            geom->getBounds().left;
        gridBounds.top =
            pageSize * Math::Floor( ( gridBounds.top - geom->getBounds().top ) / pageSize ) +
            geom->getBounds().top;
        gridBounds.right =
            pageSize * Math::Ceil( ( gridBounds.right - geom->getBounds().left ) / pageSize ) +
            geom->getBounds().left;
        gridBounds.bottom =
            pageSize * Math::Ceil( ( gridBounds.bottom - geom->getBounds().top ) / pageSize ) +
            geom->getBounds().top;
...
//Check that the tree is within bounds (DEBUG)
        ...
        if( pos.x < actualBounds.left - smallVal || pos.x > actualBounds.right + smallVal ||
            ...
            OGRE_EXCEPT( Exception::ERR_INVALIDPARAMS, "Tree position is out of bounds",
                         "TreeLoader::addTree()" );
        if( scale < minimumScale || scale > maximumScale )
            OGRE_EXCEPT( Exception::ERR_INVALIDPARAMS, "Tree scale out of range",
                         "TreeLoader::addTree()" );
#endif
        ...
        while( it != end )
        {
            ...
            for( int tileZ = minPageZ; tileZ <= maxPageZ; ++tileZ )
            {
                for( int tileX = minPageX; tileX <= maxPageX; ++tileX )
                {
                    ...
                    {
                        //Get tree distance
                        Ogre::Real distX = ( gridBounds.left + ( tileX * pageSize ) +
                                             ( (Real)treeList[i].xPos / 65535 ) * pageSize ) -
                                           pos.x;
                        Ogre::Real distZ = ( gridBounds.top + ( tileZ * pageSize ) +
                                             ( (Real)treeList[i].zPos / 65535 ) * pageSize ) -
                                           pos.z;
                    ...
                    if( modified )
                    {
                        Vector3 pos( gridBounds.left + ( ( 0.5f + tileX ) * pageSize ), 0,
                                     gridBounds.top + ( ( 0.5f + tileZ ) * pageSize ) );
                        geom->reloadGeometryPage( pos );
                    }
                }
            }
            ...
            for( int tileZ = minPageZ; tileZ <= maxPageZ; ++tileZ )
            {
                for( int tileX = minPageX; tileX <= maxPageX; ++tileX )
                {
                    ...
                    while( i < treeList.size() )
                    {
                        ...
                        Ogre::Real posX = ( gridBounds.left + ( tileX * pageSize ) +
                                            ( (Real)treeList[i].xPos / 65535 ) * pageSize );
                        Ogre::Real posZ = ( gridBounds.top + ( tileZ * pageSize ) +
                                            ( (Real)treeList[i].zPos / 65535 ) * pageSize );
                        if( posX >= area.left && posX <= area.right && posZ >= area.top &&
                            posZ <= area.bottom )
                        {
                            //If so, delete it
#ifdef PAGEDGEOMETRY_USER_DATA
...
#ifdef PAGEDGEOMETRY_USER_DATA
    std::vector<void *> TreeLoader3D::findTrees( const Ogre::Vector3 &position, Real radius,
                                                 Entity *type )
    {
//First convert the coordinate to PagedGeometry's local system
#    ifdef PAGEDGEOMETRY_ALTERNATE_COORDSYSTEM
        ...
        while( it != end )
        {
            ...
            for( int tileZ = minPageZ; tileZ <= maxPageZ; ++tileZ )
            {
                for( int tileX = minPageX; tileX <= maxPageX; ++tileX )
                {
                    ...
                    {
                        //Get tree distance
                        float distX = ( gridBounds.left + ( tileX * pageSize ) +
                                        ( (Real)treeList[i].xPos / 65535 ) * pageSize ) -
                                      pos.x;
                        float distZ = ( gridBounds.top + ( tileZ * pageSize ) +
                                        ( (Real)treeList[i].zPos / 65535 ) * pageSize ) -
                                      pos.z;
    ...
    void TreeIterator3D::moveNext()
    {
        ...
        if( !hasMore )
            OGRE_EXCEPT( Ogre::Exception::ERR_RENDERINGAPI_ERROR,
                         "Cannot read past end of TreeIterator list", "TreeIterator::moveNext()" );
