...
namespace Forests
{
    ...
    void PagedGeometry::setBounds( TBounds bounds )
    {
        if( !managerList.empty() )
            OGRE_EXCEPT( Ogre::Exception::ERR_RENDERINGAPI_ERROR,
                         "PagedGeometry::setBounds() cannot be called after detail levels have been "
                         "added. Call removeDetailLevels() first.",
    ...
    void PagedGeometry::reloadGeometryPage( const Vector3 &p )
    {
        if( !pageLoader )
            return;
    ...
    void PagedGeometry::reloadGeometryPages( const TBounds &area )
    {
        ...
        if( localArea.left > localArea.right )
            std::swap( localArea.left, localArea.right );
        if( localArea.top > localArea.bottom )
            std::swap( localArea.top, localArea.bottom );
                                      ...
                                      bool &enableCache, GeometryPageManager *prevManager )
    {
        ...
        int x1 =
            (int)Math::Floor( ( ( camPos.x - cacheDist ) - gridBounds.left ) / mainGeom->getPageSize() );
        int x2 =
            (int)Math::Floor( ( ( camPos.x + cacheDist ) - gridBounds.left ) / mainGeom->getPageSize() );
        int z1 =
            (int)Math::Floor( ( ( camPos.z - cacheDist ) - gridBounds.top ) / mainGeom->getPageSize() );
        int z2 =
            (int)Math::Floor( ( ( camPos.z + cacheDist ) - gridBounds.top ) / mainGeom->getPageSize() );
    ...
    void GeometryPageManager::preloadGeometry( const TBounds &area )
    {
        ...
        //Determine which grid block contains the top-left corner
        int x1 =
            (int)Math::Floor( geomGridX * ( loadarea.left - gridBounds.left ) / gridBounds.width() );
        ...
        //...and the bottom right
        int x2 =
            (int)Math::Floor( geomGridX * ( loadarea.right - gridBounds.left ) / gridBounds.width() );
        int z2 =
            (int)Math::Floor( geomGridZ * ( loadarea.bottom - gridBounds.top ) / gridBounds.height() );
        if( x2 < 0 )
            x2 = 0;
        else if( x2 > geomGridX - 1 )
            x2 = geomGridX - 1;
    ...
    void GeometryPageManager::_scrollGridPages( int shiftX, int shiftZ )
    {
        ...
        {  //If not, scroll the grid by the X and Y values
