                                                       ...
                                                       unsigned short lodIndex, const Renderable *rend )
{
    ...
    Ogre::SceneManager *smgr = root->_getCurrentSceneManager();
    uint currentMask = smgr->getCurrentViewport()->getVisibilityMask();
    uint mask = mDeferredShadingSystem->getViewport()->getVisibilityMask();
...
bool GBufferSchemeHandler::checkNormalMap( TextureUnitState *tus,
                                           GBufferSchemeHandler::PassProperties &props )
{
    ...
    if( isNormal )
    {
        ...
        {
            OGRE_EXCEPT( Exception::ERR_DUPLICATE_ITEM, "Multiple normal map patterns matches",
...
GBufferSchemeHandler::PassProperties GBufferSchemeHandler::inspectPass( Pass *pass,
...
void GBufferSchemeHandler::fillPass( Pass *gBufferPass, Pass *originalPass, const PassProperties &props )
{
...
GBufferSchemeHandler::GBufferSchemeHandler( DeferredShadingSystem *system ) :
    mDeferredShadingSystem( system ),
    mMaterialGenerator( system )
{
}
