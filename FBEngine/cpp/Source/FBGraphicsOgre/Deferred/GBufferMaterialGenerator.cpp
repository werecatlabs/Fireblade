...
{
public:
    GBufferMaterialGeneratorImpl( const String &baseName ) : mBaseName( baseName )
    {
        ...
        mIsGLSL = GpuProgramManager::getSingleton().isSyntaxSupported( "glsl" ) &&
                  !( GpuProgramManager::getSingleton().isSyntaxSupported( "vs_1_1" ) ||
...
GBufferMaterialGenerator::GBufferMaterialGenerator( DeferredShadingSystem *system ) :
...
GpuProgramPtr GBufferMaterialGeneratorImpl::generateVertexShader( MaterialGenerator::Perm permutation )
{
    ...
    if( mIsGLSL )
    {
        ...
        auto ptrProgram = Ogre::HighLevelGpuProgramManager::getSingleton().createProgram(
            programName, ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME, "glsl", GPT_VERTEX_PROGRAM );
        ptrProgram->setSource( programSource );
        ptrProgram->setParameter( "syntax", "glsl150" );

        const auto &params = ptrProgram->getDefaultParameters();
        params->setNamedAutoConstant( "cWorldViewProj",
    ...
    else
    {
        ...
        // Create shader object
        auto ptrProgram = Ogre::HighLevelGpuProgramManager::getSingleton().createProgram(
            programName, Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME, "cg",
...
GpuProgramPtr GBufferMaterialGeneratorImpl::generateFragmentShader( MaterialGenerator::Perm permutation )
{
    ...
    if( mIsGLSL )
    {
        ...
        HighLevelGpuProgramPtr ptrProgram = HighLevelGpuProgramManager::getSingleton().createProgram(
            programName, ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME, "glsl",
    ...
    else
    {
        ...
        for( uint32 i = 0; i < numTextures; i++ )
        {
            if( mIsSm4 )
            {
                ss << "	uniform sampler2D sTex" << i << " : register(s" << samplerNum++ << "),"
                   << std::endl;
            }
            else
            {
                ss << "	uniform sampler sTex" << i << " : register(s" << samplerNum++ << "),"
                   << std::endl;
            }
        }
        if( numTextures == 0 || permutation & GBufferMaterialGenerator::GBP_HAS_DIFFUSE_COLOUR )
    ...
    MaterialGenerator::Perm permutation )
{
    ...
    Ogre::MaterialPtr matPtr = MaterialManager::getSingleton().create(
