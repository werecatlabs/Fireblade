...
void DLight::createSphere( float radius, int nRings, int nSegments )
{
    ...
    GeomUtils::createSphere( mRenderOp.vertexData, mRenderOp.indexData, radius, nRings, nSegments,
                             false  // no normals
                             ,
...
void DLight::createCone( float radius, float height, int nVerticesInBase )
{
    ...
    GeomUtils::createCone( mRenderOp.vertexData, mRenderOp.indexData, radius, height, nVerticesInBase );

    // Set bounding box and sphere
    setBoundingBox(
        AxisAlignedBox( Vector3( -radius, 0, -radius ), Vector3( radius, height, radius ) ) );
...
void DLight::getWorldTransforms( Matrix4 *xform ) const
{
    ...
    {
        xform->makeTransform( mParentLight->getDerivedPosition(), Vector3::UNIT_SCALE,
                              Quaternion::IDENTITY );
    }
}
...
bool DLight::isCameraInsideLight( Ogre::Camera *camera )
{
    ...
    case Ogre::Light::LT_SPOTLIGHT:
    {
        ...
        return ( distanceFromLight <=
                 ( mParentLight->getAttenuationRange() + clipRangeFix.length() ) ) &&
               ( angle <= attAngle );
    }
    default:
        // Please the compiler
...
bool DLight::getCastChadows() const
{
    return mParentLight->_getManager()->isShadowTechniqueInUse() &&
           ...
           ( mParentLight->getType() == Light::LT_DIRECTIONAL ||
             mParentLight->getType() == Light::LT_SPOTLIGHT );
}
