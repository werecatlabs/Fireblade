...
void GeomUtils::createSphere( const String &strName, float radius, int nRings, int nSegments,
                              bool bNormals, bool bTexCoords )
{
    ...
    createSphere( pSphere->sharedVertexData, pSphereVertex->indexData, radius, nRings, nSegments,
    ...
    pSphere->_setBounds(
        AxisAlignedBox( Vector3( -radius, -radius, -radius ), Vector3( radius, radius, radius ) ),
        false );
...
void GeomUtils::createSphere( VertexData *&vertexData, IndexData *&indexData, float radius, int nRings,
                              int nSegments, bool bNormals, bool bTexCoords )
{
    ...
    size_t currOffset = 0;
    // positions
    vertexDecl->addElement( 0, currOffset, VertexElementType::VET_FLOAT3,
    ...
    if( bTexCoords )
    {
        vertexDecl->addElement( 0, currOffset, VertexElementType::VET_FLOAT2,
    ...
    HardwareVertexBufferSharedPtr vBuf = HardwareBufferManager::getSingleton().createVertexBuffer(
        vertexDecl->getVertexSize( 0 ), vertexData->vertexCount, HardwareBuffer::HBU_STATIC_WRITE_ONLY,
        false );
    ...
    indexData->indexBuffer = HardwareBufferManager::getSingleton().createIndexBuffer(
        HardwareIndexBuffer::IT_16BIT, indexData->indexCount, HardwareBuffer::HBU_STATIC_WRITE_ONLY,
        false );
...
void GeomUtils::createQuad( VertexData *&vertexData )
{
    ...
    vertexDecl->addElement( 0, 0, VertexElementType::VET_FLOAT3, VertexElementSemantic::VES_POSITION );
...
void GeomUtils::createCone( const Ogre::String &strName, float radius, float height,
                            int nVerticesInBase )
{
    MeshPtr pCone = MeshManager::getSingleton().createManual(
        strName, ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME );
    ...
    // the original code was missing this line:
    pCone->_setBounds(
        AxisAlignedBox( Vector3( -radius, 0, -radius ), Vector3( radius, height, radius ) ), false );
...
void GeomUtils::createCone( Ogre::VertexData *&vertexData, Ogre::IndexData *&indexData, float radius,
