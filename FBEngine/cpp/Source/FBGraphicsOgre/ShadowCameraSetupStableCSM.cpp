...
#include "FBGraphicsOgre/Addons/CSMGpuConstants.h"
...
namespace fb
{
    ...
    void StableCSMShadowCameraSetup::calculateSplitPoints( size_t cascadeCount,
                                                           Ogre::Real firstSplitDist, Ogre::Real farDist,
                                                           Ogre::Real lambda )
    {
        if( cascadeCount < 2 )
        ...
        {
            auto fraction = ( Ogre::Real )( i - 1 ) / ( mCascadeCount - 1 );
            auto logDist = firstSplitDist * Ogre::Math::Pow( farDist / firstSplitDist, fraction );
            auto linDist = firstSplitDist + fraction * ( farDist - firstSplitDist );
            auto splitPoint = linDist + lambda * ( logDist - linDist );
    ...
    void StableCSMShadowCameraSetup::setSplitPadding( Ogre::Real pad )
    {
        mSplitPadding = pad;
    }

    Ogre::Real StableCSMShadowCameraSetup::getSplitPadding() const
    {
        return mSplitPadding;
    }

    size_t StableCSMShadowCameraSetup::getCascadeCount() const
    {
        return mCascadeCount;
    }

    void StableCSMShadowCameraSetup::getShadowCamera( const Ogre::SceneManager *sm,
                                                      const Ogre::Camera *cam, const Ogre::Viewport *vp,
                                                      ...
                                                      size_t iteration ) const
    {
        ...
        auto nearDist = mSplitPoints[iteration];
        auto farDist = mSplitPoints[iteration + 1];
    ...
    const StableCSMShadowCameraSetup::SplitPointList &StableCSMShadowCameraSetup::getSplitPoints() const
    {
        return mSplitPoints;
    }

    void StableCSMShadowCameraSetup::getShadowCameraForCascade(
        const Ogre::SceneManager *sm, const Ogre::Camera *cam, const Ogre::Viewport *vp,
        const Ogre::Light *light, Ogre::Camera *texCam, size_t iteration, Ogre::Real nearSplit,
        Ogre::Real farSplit ) const
    {
        ...
        if( light->getType() == Ogre::Light::LT_DIRECTIONAL )
        {
            ...
            float dist = ( topLeftFar.squaredLength() - bottomRightNear.squaredLength() ) /
                         ( 2 * ( topLeftFar.z - bottomRightNear.z ) );
            ...
            Ogre::Vector3 localPos( 0, 0,
                                    -dist );  // we have now found the point along frustum center which
                                              // is equi-distant to the opposing corner positions

            Ogre::Real diameter = 2.0f * topLeftFar.distance( -localPos );
            pos = cam->getDerivedPosition() + cam->getDerivedOrientation() * localPos;

            diameter *= (Ogre::Real)1.01;  // allow some boundary pixels for filtering etc. TODO - make
                                           // this a user constant
            ...
            Ogre::Real worldTexelSize =
                ( texCam->getOrthoWindowWidth() ) / texCam->getViewport()->getActualWidth();
