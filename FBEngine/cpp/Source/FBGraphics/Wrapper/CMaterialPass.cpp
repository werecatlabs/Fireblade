...
namespace fb
{
    namespace render
    {
        ...
        String CMaterialPass::toJson() const
        {
            auto data = toData();
            auto passData = data->getDataAsType<data::material_pass>();
            return DataUtil::toString( passData, true );
        }

        SmartPtr<IData> CMaterialPass::toData() const
        {
        ...
        void CMaterialPass::fromData( SmartPtr<IData> data )
        {
            auto passData = data->getDataAsType<data::material_pass>();

            auto &d = passData->diffuse;
            auto &e = passData->emissive;

            auto diffuse = ColourF( d.x, d.y, d.z, d.w );
            auto emmissive = ColourF( e.x, e.y, e.z, e.w );

            setDiffuse( diffuse );
            setEmissive( emmissive );

            createTextureSlots();

            auto textureIndex = static_cast<size_t>( 0 );
            auto &textures = passData->textures;
            for( auto &texture : textures )
            {
                if( auto p = getTexturesPtr() )
                {
                    auto &texs = *p;
                    if( textureIndex < texs.size() )
                    {
                        auto pTexture = texs[textureIndex];

                        auto pTextureData = fb::make_ptr<Data<data::material_texture>>();
                        pTextureData->setData( &texture );
                        pTexture->fromData( pTextureData );
                    }
                }
