...
namespace fb
{
    namespace render
    {
        ...
        void CMaterial::setTexture( const String &fileName, u32 layerIdx )
        {
            try
            {
                if( !StringUtil::isNullOrEmpty( fileName ) )
                {
                    ...
                    auto texture = resourceDatabase->loadResourceByType<render::ITexture>( fileName );
                    if( texture )
                    {
        ...
        void CMaterial::setCubicTexture( Array<SmartPtr<render::ITexture>> textures,
                                         u32 layerIdx /*= 0 */ )
        {
            auto applicationManager = core::IApplicationManager::instance();
            auto graphicsSystem = applicationManager->getGraphicsSystem();
        ...
        IMaterial::MaterialType CMaterial::getMaterialType() const
        {
            ...
            return IMaterial::MaterialType::Standard;
        }
