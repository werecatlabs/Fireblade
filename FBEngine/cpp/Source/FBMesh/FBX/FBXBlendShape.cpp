...
#include <FBMesh/FBX/FBXBlendShape.h>
#include <FBMesh/FBX/FBXSubMesh.h>
...
{
    // Constructor
    FBXBlendShape::FBXBlendShape(FbxNode* pNode, const FbxAMatrix& mat)
    {
        clear();
        m_pNode = pNode;
        m_bindPose = mat;
    ...
    // Destructor
    FBXBlendShape::~FBXBlendShape()
    {
        clear();
    }

    // Clear blend shape data
    void FBXBlendShape::clear()
    {
        m_pNode = NULL;
        m_origWeights.clear();
        m_poseGroups.clear();
    ...
    void FBXBlendShape::getKeyedFrames(std::vector<int>& keyedFrames )
    {
    ...
    bool FBXBlendShape::loadPoses(ParamList &params, std::vector<vertex> &vertices,long numVertices,long offset,long targetIndex)
    {
    ...
    Track FBXBlendShape::loadTrack(float start,float stop,float rate,ParamList& params,int targetIndex, int startPoseId)
    {
        std::string msg;
    ...
    vertexKeyframe FBXBlendShape::loadKeyframe(float time, poseGroup& pg, int startPoseId )
    {
        vertexKeyframe key;
        key.time = time;
        key.poserefs.clear();
    ...
    HashMap<int,poseGroup>& FBXBlendShape::getPoseGroups()
    {
        return m_poseGroups;
    }
