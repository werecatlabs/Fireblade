...
namespace fb
{
	...
	void Mesh::clear()
	{
		...
		std::map<FBXBlendShape*, bool> deletedBlendShapes;
		...
		std::map<FBXBlendShape*, bool>::iterator it = deletedBlendShapes.begin();
		...
		for (int iGeom = 0; iGeom < params.pScene->GetGeometryCount(); ++iGeom)
		{
			...
			if (isVisible(pNode))
			{
				if (pFBXMesh->GetShapeCount() > 0)
				{
					...
					pBlendShape = new FBXBlendShape(pNode, bind);
	...
	bool Mesh::loadBlendShapes(ParamList& params)
	{
		...
		else
		{
			...
			{
				FBXSubMesh* pSubmesh = m_submeshes[i];
	...
	bool Mesh::loadBSClipAnim(std::string clipName, float start, float stop, float rate, ParamList& params, Animation& a)
	{
		...
		else
		{
			...
			{
				FBXSubMesh* pSubmesh = m_submeshes[j];
				if (pSubmesh->m_pBlendShape)
				{
	...
	void ReadNormal(FbxMesh* inMesh, int inCtrlPointIndex, int inVertexCounter, FbxVector4& outNormal)
	{
		...
		{
			throw Exception("Invalid Normal Number");
		}
	...
	void ReadTangent(FbxMesh* inMesh, int inCtrlPointIndex, int inVertexCounter, FbxVector4& outNormal)
	{
		...
		{
			throw Exception("Invalid Tangent Number");
		}
	...
	bool Mesh::createSubmeshes(FbxNode* pNode, FbxMesh* pMesh, ParamList& params)
	{
		for (int matIndex = 0; matIndex < pNode->GetMaterialCount(); ++matIndex)
		{
			...
			{
				FBXSubMesh* pSubmesh = new FBXSubMesh();
	...
	bool Mesh::loadMeshTrack(Animation& a, std::vector<float>& times, fb::ParamList& params)
	{
		size_t i;
		bool stat;
		// create a new track
	...
	bool Mesh::loadSubmeshTracks(Animation& a, std::vector<float>& times, fb::ParamList& params)
	{
		int i, j;
		bool stat;
	...
	bool Mesh::createOgrePoses(Ogre::MeshPtr pMesh, ParamList& params)
	{
		...
		if (params.useSharedGeom)
		{
			...
			for (size_t i = 0; i < m_sharedGeom.dagMap.size(); i++)
			{
				FBXBlendShape* pBS = m_sharedGeom.dagMap[i].pBlendShape;
		...
		else
		{
			...
			{
				FBXBlendShape* pBS = m_submeshes[i]->m_pBlendShape;
