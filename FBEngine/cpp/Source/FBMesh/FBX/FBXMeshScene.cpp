...
namespace fb
{
    ...
    FxOgreTransform FBXMeshScene::getDefaultTransForm()
    {
    ...
    void FBXMeshScene::setSceneTransform(const FxOgreTransform &trans)
    {
        m_scene_trans = trans;
    }
    void FBXMeshScene::addLight(FxOgreLight& light)
    {
        id_counter++;
        light.node.id = id_counter;
        m_lights.push_back(light);
    }
    void FBXMeshScene::addCamera(FxOgreCamera& camera)
    {
        id_counter++;
	...
	int FBXMeshScene::getNumMeshes()
	{
		return static_cast<int>(m_meshes.size());
	}

	void FBXMeshScene::clearMeshes()
	{
		m_meshes.clear();
	}

	void FBXMeshScene::addMesh(FxOgreMesh& mesh)
    {
        id_counter++;
    ...
    std::string FBXMeshScene::getLightTypeString(FxOgreLightType type)
    {
        switch( type )
        {
    ...
    tinyxml2::XMLElement* FBXMeshScene::writeNodeData(tinyxml2::XMLDocument* pXmlDoc, tinyxml2::XMLElement* parent, const FxOgreNode &node)
    {
        auto& xmlDoc = *pXmlDoc;
    ...
    bool FBXMeshScene::writeSceneFile(ParamList &params)
    {
