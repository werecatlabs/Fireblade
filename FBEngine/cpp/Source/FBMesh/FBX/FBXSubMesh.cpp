...
namespace fb
{
    /***** Class Submesh *****/
    // constructor
    FBXSubMesh::FBXSubMesh(const std::string& name)
    {
        m_pBlendShape = NULL;
        clear();
        m_name = name;
...
//			delete m_pBlendShape;
    ...
    long FBXSubMesh::numTriangles()
    {
        return m_numTriangles;
    }

    // return number of vertices composing the mesh
    long FBXSubMesh::numVertices()
    {
        return m_numVertices;
    }

    // return submesh name
    std::string& FBXSubMesh::name()
    {
        return m_name;
    }
    ...
    bool FBXSubMesh::loadMaterial(FbxSurfaceMaterial* pGameMaterial, std::vector<int>& uvsets, ParamList& params)
    {
        ...
        Material* pMaterial = FBXMaterialSet::getSingleton().getMaterial(name);
        ...
        else
        {
            ...
            m_pMaterial = pMaterial;
            FBXMaterialSet::getSingleton().addMaterial(pMaterial);
        }
        //loading complete
    ...
    bool FBXSubMesh::load(FbxNode* pNode, FbxMesh* pMesh, const std::vector<face>& faces, const std::vector<vertexInfo>& vertInfo, const std::vector<FbxVector4>& points,
    ...
    bool FBXSubMesh::loadKeyframe(Track& t, float time, ParamList& params)
    {
    ...
    bool FBXSubMesh::createOgreSubmesh(Ogre::MeshPtr pMesh, const ParamList& params)
    {
        size_t i, j;
        bool stat;
    ...
    bool FBXSubMesh::createOgreVertexBuffer(Ogre::SubMesh* pSubmesh, Ogre::VertexDeclaration* pDecl, const std::vector<vertex>& vertices)
    {
	...
	fb::FBXBlendShape* FBXSubMesh::getBlendShape()
	{
