...
#        define NOMINMAX
#    endif
#    include <FBImGui/Extern/Dirent/dirent.h>
#    include <windows.h>
#else
#    include <dirent.h>
    ...
    namespace ui
    {
                ...
                if(i != current_dirlist.size() - 1)
                {
                       ...
                       ( nw_size.x - style.WindowPadding.x * 3.0 ))
                    {
                        ...
                        if(ImGui::BeginPopup( "##NavBarDropboxPopup" ))
                        {
                            ...
                            if(ImGui::BeginListBox( "##NavBarDropBox",
                                                    ImVec2( 0, list_item_height * 5 ) ))
                            {
                                ImGui::PushStyleColor( ImGuiCol_Text,
                                                       ImVec4( 0.882f, 0.745f, 0.078f, 1.0f ) );
                                for(Array<std::string>::size_type j = i + 1;
                                    j < current_dirlist.size(); j++)
                                {
                    ...
                    ImGui::PushStyleColor( ImGuiCol_Button, ImVec4( 1.0f, 1.0f, 1.0f, 0.01f ) );
                    ImGui::PushStyleColor( ImGuiCol_Text, ImVec4( 1.0f, 1.0f, 1.0f, 1.0f ) );
                    ImGui::ArrowButtonEx( "##Right", ImGuiDir_Right,
                                          ImVec2( frame_height, frame_height ),
        ...
        bool ImGuiFileBrowser::onDirClick( int idx )
        {
            ...
            if(name == "..")
            {
                ...
                new_path = new_path.substr(
                    0, new_path.find_last_of( '/' ) + 1 ); // Also include a trailing '/'
            }
            else
            {
        ...
        bool ImGuiFileBrowser::validateFile()
        {
            ...
            std::string file_ext = idx == std::string::npos
                                       ? ""
                                       : selected_fn.substr( idx, selected_fn.length() - idx );

            std::transform( file_ext.begin(), file_ext.end(), file_ext.begin(),
                            []( unsigned char c ) { return std::tolower( c ); } );

            return ( std::find( valid_exts.begin(), valid_exts.end(), file_ext ) !=
                     valid_exts.end() );
        }
