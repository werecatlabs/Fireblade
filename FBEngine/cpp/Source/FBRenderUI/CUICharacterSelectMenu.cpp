...
namespace fb::ui
{
    ...
    auto CUICharacterSelectMenu::handleEvent( const SmartPtr<IInputEvent> &event ) -> bool
    {
        if( !isEnabled() )
        {
            return false;
    ...
    auto CUICharacterSelectMenu::inputEvent( const SmartPtr<IInputEvent> &event ) -> bool
    {
        Parameters params;
        // params.set_used(1);
    ...
    void CUICharacterSelectMenu::populateMenuItemList()
    {
        ...
        m_uiNumListItems = static_cast<s32>( m_menuItems.size() );  // default value
    ...
    auto CUICharacterSelectMenu::getNumMenuItems() const -> int
    {
        // populateMenuItemList();
        return static_cast<s32>( m_menuItems.size() );
    }

    auto CUICharacterSelectMenu::getCurrentCursorIndex() const -> u32
    {
    ...
    void CUICharacterSelectMenu::WrapCursor( s32 cursorIndex )
    {
        ...
        if( m_iItemIdx <= diff && diff >= 0 )
        {
            ...
            else if( m_cursorIdx[cursorIndex] >= static_cast<s32>( m_menuItems.size() ) )
            {
                ...
                m_cursorIdx[cursorIndex] = static_cast<s32>( m_menuItems.size() ) - 1;
            }
    ...
    auto CUICharacterSelectMenu::getCursorPosition( u32 cursorIdx ) const -> u32
    {
    ...
    auto CUICharacterSelectMenu::getNumListItems() const -> u32
    {
    ...
    auto CUICharacterSelectMenu::getCurrentItemIndex() const -> u32
    {
        return m_iItemIdx;
    }
