...
namespace fb::ui
{
    ...
    void CUIFadeEffect::OnUpdateFadeInState()
    {
        ...
        m_alpha -= 0.5f * static_cast<f32>( dt );
        if( m_alpha > 1.0f )
        {
            m_alpha = 1.0f;
        }
    ...
    auto CUIFadeEffect::GetStateFromName( const String &stateName ) const -> u8
    {
        if( stateName == ( "Idle" ) )
        {
            return FS_IDLE;
    ...
    auto CUIFadeEffect::GetStateNameFromId( u8 stateId ) const -> String
    {
        switch( stateId )
        {
        case FS_IDLE:
    ...
    auto CUIFadeEffect::getFSM() -> SmartPtr<IFSM> &
    {
        return m_fsm;
    }
