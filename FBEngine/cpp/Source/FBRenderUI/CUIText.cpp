...
namespace fb::ui
{
    ...
    auto CUIText::getText() const -> String
    {
        if( auto stateContext = getStateContext() )
        {
            if( auto state = stateContext->getStateByType<UIElementState>() )
    ...
    auto CUIText::getTextSize() const -> f32
    {
        if( auto stateContext = getStateContext() )
        {
            if( auto state = stateContext->getStateByType<UIElementState>() )
    ...
    auto CUIText::getVerticalAlignment() const -> u8
    {
        if( m_overlayText )
        {
    ...
    auto CUIText::getOverlayText() const -> SmartPtr<render::IOverlayElementText>
    {
    ...
    void CUIText::handleStateChanged( SmartPtr<IState> &state )
    {
        ...
        if( auto overlayText = getOverlayText() )
        {
            ...
            overlayText->setCharHeight( static_cast<f32>( textSize ) / 1080.0f );
        }
