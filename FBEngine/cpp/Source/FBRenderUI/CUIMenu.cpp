...
namespace fb::ui
{
    ...
    auto CUIMenu::handleEvent( const SmartPtr<IInputEvent> &event ) -> bool
    {
        if( !isEnabled() )
        {
            return false;
    ...
    void CUIMenu::incrementCursor()
    {
        ...
        while( !isMenuItemEnabled && counter < m_uiNumMenuItems )
        {
            ...
            auto pPrevMenuItem = reinterpret_cast<CUIMenuItem *>( m_aMenuItemList[m_prevCursorIdx] );
            auto pCurMenuItem = reinterpret_cast<CUIMenuItem *>( m_aMenuItemList[m_cursorIdx] );
    ...
    auto CUIMenu::getNumMenuItems() const -> int
    {
        // populateMenuItemList();
        return static_cast<s32>( m_aMenuItemList.size() );
    }
    ...
    void CUIMenu::WrapCursor()
    {
        ...
        if( m_iItemIdx <= diff && diff >= 0 )
        {
            ...
            else if( m_cursorIdx >= static_cast<s32>( m_aMenuItemList.size() ) )
            {
                ...
                m_cursorIdx = static_cast<s32>( m_aMenuItemList.size() ) - 1;
            }
    ...
    auto CUIMenu::getCursorPosition() const -> u32
    {
        return m_cursorIdx;
    }
    ...
    void CUIMenu::onChildChangedState( CUIElement *child )
    {
        ...
        for( auto menuItem : m_aMenuItemList )
        {
    ...
    auto CUIMenu::getNumListItems() const -> u32
    {
    ...
    auto CUIMenu::getCurrentItemIndex() const -> u32
    {
        return m_iItemIdx;
    }
    ...
    void CUIMenu::addMenuItem( SmartPtr<IUIElement> item )
    {
    }

    void CUIMenu::removeMenuItem( SmartPtr<IUIElement> item )
    {
    }

    fb::Array<fb::SmartPtr<fb::ui::IUIElement>> CUIMenu::getMenuItems() const
    {
        return Array<fb::SmartPtr<fb::ui::IUIElement>>();
    }

    void CUIMenu::setMenuItems( Array<SmartPtr<IUIElement>> val )
    {
    }

    String CUIMenu::getLabel() const
    {
        return m_label;
    }

    void CUIMenu::setLabel( const String &label )
    {
        m_label = label;
    }
