...
namespace fb::ui
{
    ...
    auto CUIScrollingText::handleEvent( const SmartPtr<IInputEvent> &event ) -> bool
    {
        if( !isEnabled() )
        {
            return false;
    ...
    auto CUIScrollingText::getNumMenuItems() const -> int
    {
    ...
    void CUIScrollingText::WrapCursor()
    {
        ...
        if( m_iItemIdx <= diff && diff >= 0 )
        {
            ...
            else if( m_cursorIdx >= static_cast<s32>( m_elements.size() ) )
            {
                ...
                m_cursorIdx = static_cast<s32>( m_elements.size() ) - 1;
            }
        ...
        {
            if( m_cursorIdx >= static_cast<s32>( m_elements.size() ) || m_cursorIdx >= m_uiNumListItems )
            {
                m_iItemIdx = 0;
                m_cursorIdx = 0;
            }
            ...
            {
                m_cursorIdx = MathI::min( static_cast<s32>( m_elements.size() ), m_uiNumListItems ) - 1;
            }
        }
    ...
    auto CUIScrollingText::getCursorPosition() const -> u32
    {
        return m_cursorIdx;
    }
    ...
    auto CUIScrollingText::getNumListItems() const -> u32
    {
    ...
    auto CUIScrollingText::getCurrentItemIndex() const -> u32
    {
        return m_iItemIdx;
    }
