...
void DecalCursor::init( const Ogre::Vector2 &size, const std::string &tex )
{
    ...
    /* Commented out for general use. If you are using mouse picking in your scene, set this to an
     * unselectable flag otherwise you may find the frustum in your ray queries. */
    // m_frustProj->setQueryFlags(Sanguis::UNSELECTABLE);

    // set given values
    setSize( size );
    m_sTextureName = tex;  // texture to apply

    // load the images for the decal and the filter
    // Ogre::TextureManager::getSingleton().load(m_sTextureName,
    // Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME, Ogre::TEX_TYPE_2D, 1);
...
void DecalCursor::setTextureName( const std::string &val )
{
    m_sTextureName = val;
}
