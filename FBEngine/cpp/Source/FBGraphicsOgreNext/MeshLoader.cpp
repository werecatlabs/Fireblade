    ...
    namespace render
    {
        ...
        void createV2Mesh( Ogre::Mesh *mesh, SmartPtr<IMesh> fbmesh )
        {
            ...
            for( size_t subMeshIdx = 0; subMeshIdx < numSubMeshes; ++subMeshIdx )
            {
                ...
                constexpr auto index16BitClamp = std::numeric_limits<u16>::max() - 1;
                
                auto buff_type = fbVertexCount > index16BitClamp ? Ogre::IndexBufferPacked::IT_32BIT
                                                                 : Ogre::IndexBufferPacked::IT_16BIT;

                auto indexBuffer = vaoManager->createIndexBuffer(
                    buff_type, iBufSize, Ogre::BT_IMMUTABLE, fbIndexBuffer->getIndexData(), false );
        ...
        void MeshLoader::loadFBMesh( Ogre::MeshPtr mesh, const String &meshPath )
        {
            ...
            if( fbMeshStream )
            {
                ...
                //auto v1Mesh = convertFBMeshToOgreMesh( meshPath, fbMesh );

                //auto halfPos = false;
                //auto halfTexCoords = false;
                //auto qTangents = false;
                //auto halfPose = false;

                //mesh->importV1( v1Mesh.get(), halfPos, halfTexCoords, qTangents, halfPose );
            }
        }

        Ogre::Mesh *MeshLoader::loadFBMesh( SmartPtr<IStream> stream )
        {
            return nullptr;
        }
        ...
        void MeshLoader::loadFBMesh( Ogre::MeshPtr meshPtr, SmartPtr<IMesh> mesh )
        {
            ...
            auto ogreAabb =
                Ogre::AxisAlignedBox( Ogre::Vector3( minimum.X(), minimum.Y(), minimum.Z() ),
        ...
        Ogre::v1::MeshPtr MeshLoader::convertFBMeshToOgreMesh( const String &newMeshName,
                                                               SmartPtr<IMesh> mesh )
        {
            if( !mesh )
            {
            ...
            auto defaultHardwareBufferManager =
                Ogre::v1::DefaultHardwareBufferManager::getSingletonPtr();
            ...
            for( size_t subMeshIdx = 0; subMeshIdx < numSubMeshes; ++subMeshIdx )
            {
                ...
                if( texCoordElem0 )
                {
                    decl->addElement( iBinding, offset, Ogre::VET_FLOAT3, Ogre::VES_TEXTURE_COORDINATES,
                                      0 );
                    offset += sizeof( f32 ) * 3;
                }
        ...
        Ogre::IndexBufferPacked *MeshLoader::importFromV1( SmartPtr<IIndexBuffer> indexData )
        {
            /*
        ...
        SmartPtr<IMesh> MeshLoader::loadEngineMesh( const String &meshName )
        {
            /*
            String fileExt = Path::getFileExtension(meshName);
            if(fileExt==(".mesh"))
        ...
        SmartPtr<IMesh> MeshLoader::load( const String &meshName, SmartPtr<ISceneNode> fbParent,
        ...
        SmartPtr<ISceneNode> MeshLoader::loadNode( aiNode *node, SmartPtr<ISceneNode> fbNode,
                                                   SmartPtr<render::IGraphicsScene> smgr )
        {
            /*
        ...
        void MeshLoader::loadDataFromNode( const aiScene *mScene, const aiNode *pNode,
                                           const String &mDir )
        {
            // auto engine = core::IApplicationManager::instance();
                                        ...
                                        const String &mDir )
        {
            ...
            if( mBonesByName.size() && !mesh->HasBones() )
            {
                if( !mQuietMode )
                {
                    FB_LOG_MESSAGE( "MeshLoader", String( "Skipping Mesh " ) +
                                                      String( mesh->mName.data ) +
            ...
            if( norm )
            {
                if( !mQuietMode )
                {
                    FB_LOG_MESSAGE( "MeshLoader",
                                    StringUtil::toString( mesh->mNumVertices ) + " normals" );
                }
            ...
            */
            ...
            float *vdata = (float *)vertexData->createVertexData();
            ...
            u16 *idata = (u16 *)indexBuffer->createIndexData();

            // poke in the face data
            for( u32 i = 0; i < mesh->mNumFaces; ++i )
            {
                //		wxASSERT(f->mNumIndices == 3);
                *idata++ = (u16)f->mIndices[0];
                *idata++ = (u16)f->mIndices[1];
                *idata++ = (u16)f->mIndices[2];
                f++;
            }
        ...
        SmartPtr<IMeshManager> MeshLoader::getMeshMgr() const
        {
            return m_meshMgr;
        }
