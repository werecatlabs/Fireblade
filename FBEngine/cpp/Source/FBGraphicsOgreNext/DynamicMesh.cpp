...
namespace fb::render
{
    ...
    //--------------------------------------------
    auto DynamicMesh::getOwner() const -> Ogre::SceneNode *
    {
        return m_owner;
    }
    ...
    //--------------------------------------------
    auto DynamicMesh::getSubMesh() const -> SmartPtr<ISubMesh>
    {
        return m_fbMesh;
    }
    ...
    */
    ...
    {
        // int vertexCount = m_fbMesh->getVertexBuffer()->getNumVerticies();
        // int indexCount = m_fbMesh->getIndexBuffer()->getNumIndices();
        ...
        // vertexBuffer->getVertexDeclaration()->findElementBySemantic(fb::VertexDeclaration::VES_POSITION);
        ...
        // vertexBuffer->getVertexDeclaration()->findElementBySemantic(fb::VertexDeclaration::VES_TEXTURE_COORDINATES,
        // 0); const SmartPtr<IVertexElement> texCoordElem1 =
        // vertexBuffer->getVertexDeclaration()->findElementBySemantic(fb::VertexDeclaration::VES_TEXTURE_COORDINATES,
        // 1);
        ...
        // bool useWords = (vertexCount >= std::numeric_limits<unsigned short>::max()) ? false :
        // true; unsigned short *pWords = useWords ? static_cast<unsigned
        // short*>(ibuf->lock(Ogre::HardwareBuffer::HBL_DISCARD)) : NULL; unsigned int *pDWords =
        // useWords ? NULL : static_cast<unsigned
        // int*>(ibuf->lock(Ogre::HardwareBuffer::HBL_DISCARD));
