...
namespace fb
{
    namespace render
    {
        ...
        void CResourceGroupManager::loadTextureCache( void )
        {
#if !OGRE_NO_JSON
            Ogre::ArchiveManager &archiveManager = Ogre::ArchiveManager::getSingleton();
            ...
            try
            {
                ...
                if( rwAccessFolderArchive->exists( filename ) )
                {
                    ...
                    if( !fileData.empty() )
                    {
                        ...
                        textureManager->importTextureMetadataCache( stream->getName(), &fileData[0],
                                                                    false );
                    }
                }
                else
        ...
        void CResourceGroupManager::saveTextureCache( void )
        {
            auto root = Ogre::Root::getSingletonPtr();
                ...
                if( textureManager )
                {
                    ...
                    if( file.is_open() )
                        file.write( jsonString.c_str(),
                                    static_cast<std::streamsize>( jsonString.size() ) );
                    file.close();
                }
            }
        }

        void CResourceGroupManager::loadHlmsDiskCache( void )
        {
                        ...
                        try
                        {
                            if( rwAccessFolderArchive->exists( filename ) )
                            {
                                Ogre::DataStreamPtr diskCacheFile =
                                    rwAccessFolderArchive->open( filename );
        ...
        void CResourceGroupManager::saveHlmsDiskCache( void )
        {
            auto root = Ogre::Root::getSingletonPtr();
        ...
        CResourceGroupManager::ResourceLoadJob::ResourceLoadJob()
        {
        }
...
#    if FB_BUILD_RENDERER_DX11
                    ...
                    m_resourceGroupManager->parseScripts( gpuProgramManager, hlslProgramExt,
                                                          defaultGrp );
#    endif
#endif
...
#    endif
                    ...
                    m_resourceGroupManager->parseScripts( gpuProgramManager, metalProgramExt,
                                                          defaultGrp );
#endif
                }
                catch( std::exception &e )
        ...
        CResourceGroupManager *CResourceGroupManager::ResourceLoadJob::getResourceGroupManager() const
        {
            return m_resourceGroupManager;
        }

        void CResourceGroupManager::ResourceLoadJob::setResourceGroupManager(
            CResourceGroupManager *val )
        {
