...
namespace Colibri
{
    ...
    //-------------------------------------------------------------------------
    const Ogre::ColourValue &Renderable::getColour() const
    {
        return m_colour;
    }
    //-------------------------------------------------------------------------
    bool Renderable::getOverrideSkinColour() const
    {
        return m_overrideSkinColour;
    }
    //-------------------------------------------------------------------------
    ...
    //-------------------------------------------------------------------------
    void Renderable::_setSkinPack( SkinInfo const *colibri_nonnull const *colibri_nullable skinInfos )
    {
        for( size_t i = 0; i < States::NumStates; ++i )
        {
            if( skinInfos[i] )
    ...
    void Renderable::_addCommands( ApiEncapsulatedObjects &apiObject, bool collectingBreadthFirst )
    {
        ...
        if( m_visualsEnabled )
        {
            ...
            uint32 baseInstance =
                apiObject.hlms->fillBuffersForColibri( hlmsCache, queuedRenderable, false, firstVertex,
    ...
    //-------------------------------------------------------------------------
    void Renderable::_fillBuffersAndCommands(
    ...
    {
        _fillBuffersAndCommands( vertexBuffer, textVertBuffer, parentPos, parentCurrentScrollPos,
                                 parentRot, Ogre::Vector2::ZERO, false );
    }
}  // namespace Colibri
