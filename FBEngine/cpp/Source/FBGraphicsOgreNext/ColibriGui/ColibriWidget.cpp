...
namespace Colibri
{
    static Borders::Borders c_reciprocateBorders[Borders::NumBorders + 1u] = {
        Borders::Bottom, Borders::Right, Borders::Left, Borders::Top, Borders::NumBorders
    };
    ...
    size_t Widget::notifyParentChildIsDestroyed( Widget *childWidgetBeingRemoved )
    {
        ...
        WidgetVec::iterator itor =
            std::find( m_children.begin(), m_children.end(), childWidgetBeingRemoved );
    ...
    //-------------------------------------------------------------------------
    Widget *colibri_nullable Widget::getNextKeyboardNavigableWidget( const Borders::Borders direction )
    {
    ...
    //-------------------------------------------------------------------------
    void Widget::_setDestructionDelayed()
    {
        m_hidden = true;
    }
    //-------------------------------------------------------------------------
    ...
    //-------------------------------------------------------------------------
    void Widget::setIgnoreFromChildrenSize( bool bIgnore )
    {
        m_ignoreFromChildrenSize = bIgnore;
    }
    //-------------------------------------------------------------------------
    ...
    bool Widget::intersects( const Ogre::Vector2 &posNdc ) const
    {
        ...
        return !( posNdc.x < m_derivedTopLeft.x || posNdc.y < m_derivedTopLeft.y ||
                  posNdc.x > m_derivedBottomRight.x || posNdc.y > m_derivedBottomRight.y );
    }
    //-------------------------------------------------------------------------
                                          ...
                                          const Matrix2x3 &parentRot )
    {
        ...
        const Ogre::Vector2 outerTopLeftWithClipping =
            outerTopLeft + ( m_clipBorderTL - currentScrollPos ) * invCanvasSize2x;

        while( itor != end )
        {
            ( *itor )->_fillBuffersAndCommands( vertexBuffer, textVertBuffer, outerTopLeftWithClipping,
                                                currentScrollPos, m_derivedOrientation );
            ++itor;
        }
    }
    //-------------------------------------------------------------------------
    ...
    void Widget::setState( States::States state, bool smartHighlight )
    {
        ...
        if( m_keyboardNavigable && ( ( state == States::Disabled && oldValue != States::Disabled ) ||
    ...
    //-------------------------------------------------------------------------
    void Widget::_setTextInput( const char *text, const bool bReplaceContents,
                                const bool bCallActionListener )
    {
        COLIBRI_UNUSED( text );
        COLIBRI_UNUSED( bReplaceContents );
        COLIBRI_UNUSED( bCallActionListener );
    }
    //-------------------------------------------------------------------------
    ...
    //-------------------------------------------------------------------------
    ColibriManager *Widget::getManager()
    {
        return m_manager;
    }
    //-------------------------------------------------------------------------
