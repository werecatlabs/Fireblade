...
namespace Colibri
{
    SkinManager::SkinManager( ColibriManager *colibriManager ) : m_colibriManager( colibriManager )
    {
    }
    //-------------------------------------------------------------------------
    ...
    void SkinManager::buildGridFromEnclosingUv( Ogre::Vector4 uvTopLeftWidthHeight,
                                                ...
                                                StateInformation &stateInfo, const char *skinName,
                                                const char *filename )
    {
            ...
            borderSizeTL.y + borderSizeBR.y > uvTopLeftWidthHeight.w )
        {
            ...
            errorMsg.clear();
            errorMsg.a(
                "[SkinManager::buildGridFromEnclosingUv] Enclosing's border parameter "
                "must be less than half of the width and less than the height. "
                "Skin: ",
                skinName, " File: ", filename );
            ...
            if( itor->name.IsString() && itor->value.IsObject() )
            {
                ...
                if( itTmp != skinValue.MemberEnd() && itTmp->value.IsString() )
                {
                    ...
                    if( itSkin != m_skins.end() )
                    {
                        ...
                        if( itTmp != itBaseSkin->value.MemberEnd() && itTmp->value.IsArray() &&
                            itTmp->value.Size() == 2u && itTmp->value[0].IsUint() &&
                            itTmp->value[1].IsUint() )
                        {
                    ...
                    {
                        errorMsg.clear();
                        errorMsg.a( "[SkinManager::loadSkins]: Skin ", baseSkinName, " needed by skin ",
                                    itor->name.GetString(),
                                    " via copy_from not found! Note skins to be copied "
                                    "from must be defined first. in ",
                                    filename );
                ...
                if( itTmp != skinValue.MemberEnd() && itTmp->value.IsArray() &&
                    itTmp->value.Size() == 2u && itTmp->value[0].IsUint() && itTmp->value[1].IsUint() )
                {
                    texResolution.x = Ogre::Real( itTmp->value[0].GetUint() );
                    texResolution.y = Ogre::Real( itTmp->value[1].GetUint() );
                }
                ...
                if( itTmp != skinValue.MemberEnd() && itTmp->value.IsObject() )
                {
                    ...
                    if( itTmp != skinValue.MemberEnd() && itTmp->value.IsArray() )
                    {
                        ...
                        skinInfo.stateInfo.centerAspectRatio =
                            uvTopLeftWidthHeight.z / uvTopLeftWidthHeight.w;
                    ...
                    if( itTmp != skinValue.MemberEnd() && itTmp->value.IsArray() &&
                        ( ( itTmp->value.Size() == 2u && itTmp->value[0].IsArray() &&
                            itTmp->value[1].IsArray() ) ||
                          ( itTmp->value.Size() == 3u && itTmp->value[0].IsArray() &&
                            itTmp->value[1].IsArray() && itTmp->value[2].IsArray() ) ) )
                    {
                    ...
                    const char *gridLocations[GridLocations::NumGridLocations] = {
                        "top_left",     "top",         "top_right", "center_left", "center",
                                     ...
                                     const rapidjson::Value &skinsValue, const char *filename )
    {
        ...
        while( itor != end )
        {
            if( itor->name.IsString() && itor->value.IsObject() )
            {
                ...
                const char *states[States::NumStates] = { "disabled",
                                                          "idle",
                                                          "highlighted_cursor",
                                                          "highlighted_button",
    ...
    //-------------------------------------------------------------------------
    SkinInfo const *colibri_nullable SkinManager::findSkin( const SkinPack &pack, States::States state,
                                                            LogSeverity::LogSeverity logSeverity ) const
    {
