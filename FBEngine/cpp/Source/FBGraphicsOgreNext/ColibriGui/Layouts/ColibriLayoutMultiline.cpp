...
namespace Colibri
{
    ...
    //-------------------------------------------------------------------------
    void LayoutMultiline::clearCells()
    {
        m_cells.clear();
    }
    //-------------------------------------------------------------------------
    inline Ogre::Vector2 LayoutMultiline::getTopLeft(
        bool bVertical, GridLocations::GridLocations gridLoc, float accumOffset, float cellSize,
        float maxOtherSize, const Ogre::Vector2 &finalCellSize, const Ogre::Vector2 &halfMargin )
    {
    ...
    struct SortByPriorityLowToHigh
    {
        ...
        SortByPriorityLowToHigh( const LayoutCellVec &_cells ) : cells( _cells )
        {
        }
    ...
    struct SortByPriorityHighToLow
    {
        ...
        SortByPriorityHighToLow( const LayoutCellVec &_cells ) : cells( _cells )
        {
        }
    ...
    void LayoutMultiline::layout()
    {
        ...
        if( m_cells.size() % numLines )
        {
            ...
            logListener->log(
                "LayoutMultiline::layout: For best results m_cells.size() "
...

        {
            ...
            while( itor != endt )
            {
                ...
                maxOtherSize = std::max(
                    maxOtherSize, biggestMinSizeInColumn[!bVertical] + biggestMargin[!bVertical] );
        ...
        sizeToDistribute =
            std::max( softMaxSize[bVertical] - accumMarginSize - nonProportionalSize, sizeToDistribute );
        if( !canScroll )
        {
            sizeToDistribute =
                std::min( sizeToDistribute, hardMaxSize[bVertical] - nonProportionalSize );
        }
        ...
        const float spaceLeftForMargins = Ogre::Math::Clamp(
            hardMaxSize[bVertical] - sizeToDistribute - nonProportionalSize, 0.0f, accumMarginSize );
        ...
        for( size_t y = 0; y < numLines; ++y )
        {
            ...
            for( size_t x = 0; x < numCellsPerLine; ++x )
            {
                ...
                if( cell->m_expand[!bVertical] )
                {
                    ...
                    finalCellSize[!bVertical] =
                        maxOtherSize - std::min( otherAvailableSize, cell->m_margin[!bVertical] );
                }
    ...
    Ogre::Vector2 LayoutMultiline::getBiggestMargin( size_t columnIdx ) const
    {
        ...
        {
            const size_t numLines = std::max<size_t>( m_numLines, 1u );
            const size_t numCellsPerLine =
                Ogre::alignToNextMultiple( m_cells.size(), numLines ) / numLines;
