...
namespace Colibri
{
    ...
    //-------------------------------------------------------------------------
    void LayoutLine::clearCells()
    {
        m_cells.clear();
    }
    //-------------------------------------------------------------------------
    inline Ogre::Vector2 LayoutLine::getTopLeft( bool bVertical, GridLocations::GridLocations gridLoc,
                                                 float accumOffset, float cellSize, float maxOtherSize,
    ...
    void LayoutLine::layout()
    {
...

        {
            ...
            while( itor != end )
            {
                ...
                maxOtherSize =
                    std::max( maxOtherSize, cellMinSize[!bVertical] + cell->m_margin[!bVertical] );
    ...
    void LayoutLine::calculateCurrentSize()
    {
        ...
        m_currentSize =
            Ogre::Vector2( m_vertical ? maxedVal.x : accumVal.x, m_vertical ? accumVal.y : maxedVal.y );
        const Ogre::Vector2 layoutMargin = m_adjustableWindow ? m_margin : Ogre::Vector2::ZERO;
