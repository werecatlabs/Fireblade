    ...
    namespace detail
    {
        ...
        namespace
        {
            ...
            public:
                ...
                cache_entry get( class_id src, class_id target, class_id dynamic_id,
                                 std::ptrdiff_t object_offset ) const;

                void put( class_id src, class_id target, class_id dynamic_id,
            ...
            cache_entry cache::get( class_id src, class_id target, class_id dynamic_id,
                                    std::ptrdiff_t object_offset ) const
            {
                map_type::const_iterator i =
                    m_cache.find( key_type( src, target, dynamic_id, object_offset ) );
                return i != m_cache.end() ? i->second : cache_entry( unknown, -1 );
            }

            void cache::put( class_id src, class_id target, class_id dynamic_id,
                             std::ptrdiff_t object_offset, std::ptrdiff_t offset, int distance )
            {
                m_cache.insert( std::make_pair( key_type( src, target, dynamic_id, object_offset ),
        ...
        {
        public:
            std::pair<void *, int> cast( void *p, class_id src, class_id target, class_id dynamic_id,
        ...
        std::pair<void *, int> cast_graph::impl::cast( void *const p, class_id src, class_id target,
                                                       class_id dynamic_id,
                                                       void const *dynamic_ptr ) const
        {
            if( src == target )
            ...
            while( !q.empty() )
            {
                ...
                if( v.id == target )
                {
                    m_cache.put( src, target, dynamic_id, object_offset, (char *)qe.p - (char *)p,
                                 qe.distance );
        ...
        void cast_graph::impl::insert( class_id src, class_id target, cast_function cast )
        {
            ...
            std::vector<edge>::iterator i =
                std::lower_bound( edges.begin(), edges.end(), edge( target, 0 ) );
        ...
        std::pair<void *, int> cast_graph::cast( void *p, class_id src, class_id target,
        ...
        LUABIND_API class_id allocate_class_id( type_id const &cls )
        {
            ...
            std::pair<map_type::iterator, bool> inserted =
                registered.insert( std::make_pair( cls, id ) );
