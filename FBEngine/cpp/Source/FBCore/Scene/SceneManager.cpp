...
namespace fb::scene
{
    ...
    void SceneManager::preUpdate()
    {
        try
        {
            ...
            if( m_scene->getSceneLoadingState() == IScene::SceneLoadingState::Loaded )
            {
    ...
    auto SceneManager::getLoadPriority( SmartPtr<ISharedObject> obj ) -> s32
    {
    ...
    void SceneManager::loadScene( const String &filePath )
    {
        if( !StringUtil::isNullOrEmpty( filePath ) )
        {
            ...
            {
                stateContext->triggerEvent( IEvent::Type::Loading, sceneLoadedHash, Array<Parameter>(),
                                            this, this, nullptr );
            }
        }
    }
    ...
    auto SceneManager::getComponentFsmManager( u32 typeId ) -> SmartPtr<IFSMManager>
    {
    ...
    auto SceneManager::getCurrentScene() const -> SmartPtr<IScene>
    {
        return m_scene;
    }
    ...
    auto SceneManager::getActors() const -> Array<SmartPtr<IActor>>
    {
        Array<SmartPtr<IActor>> actors;
    ...
    auto SceneManager::createActor() -> SmartPtr<IActor>
    {
        try
        {
    ...
    auto SceneManager::addTransformComponent( SmartPtr<Transform> transformComponent ) -> u32
    {
    ...
    auto SceneManager::addComponent( SmartPtr<IComponent> component ) -> u32
    {
    ...
    auto SceneManager::removeComponent( SmartPtr<IComponent> component ) -> u32
    {
        auto handle = component->getHandle();
    ...
    void SceneManager::addSystem( u32 id, SmartPtr<IComponentSystem> system )
    {
        m_systems[id] = system;
    }
    ...
    auto SceneManager::getActor( u32 id ) const -> SmartPtr<IActor>
    {
        FB_ASSERT( id < m_actors.size() );
        return m_actors[id];
    }

    auto SceneManager::getActorByName( const String &name ) const -> SmartPtr<IActor>
    {
        auto actors = getActors();
        for( auto actor : actors )
        {
    ...
    auto SceneManager::getActorByFileId( const String &id ) const -> SmartPtr<IActor>
    {
        auto actors = getActors();
        for( auto actor : actors )
        {
    ...
    auto SceneManager::getFSM( u32 id ) const -> SmartPtr<IFSM>
    {
        FB_ASSERT( id < m_fsms.size() );
        return m_fsms[id];
    }

    auto SceneManager::createTransformComponent() -> SmartPtr<ITransform>
    {
        auto applicationManager = core::ApplicationManager::instance();
        FB_ASSERT( applicationManager );
    ...
    auto SceneManager::getTransform( u32 id ) const -> SmartPtr<Transform>
    {
        FB_ASSERT( id < m_transforms.size() );
        return m_transforms[id];
    }

    auto SceneManager::getCurrentFlags( u32 id ) const -> u32
    {
    ...
    auto SceneManager::getNewFlags( u32 id ) const -> u32
    {
    ...
    auto SceneManager::getPreviousFlags( u32 id ) const -> u32
    {
    ...
    auto SceneManager::getFlags( u32 id ) const -> u32
    {
        FB_ASSERT( id < m_actorCurrentFlags.size() );
        return m_actorCurrentFlags[id];
    }
    ...
    void SceneManager::queueProperties( SmartPtr<ISharedObject> object, SmartPtr<Properties> properties )
    {
        if( isLoaded() )
        {
    ...
    void SceneManager::makeActorTransformsDirty()
    {
        auto actors = getActors();
        for( auto actor : actors )
        {
    ...
    auto SceneManager::getStateTask() const -> Thread::Task
    {
        auto applicationManager = core::ApplicationManager::instance();
        FB_ASSERT( applicationManager );
    ...
    auto SceneManager::getSceneTask() const -> Thread::Task
    {
        auto applicationManager = core::ApplicationManager::instance();
        FB_ASSERT( applicationManager );
    ...
    auto SceneManager::getComponents() const -> Array<SmartPtr<IComponent>>
    {
    ...
    auto SceneManager::getComponents( u32 type ) const -> Array<SmartPtr<IComponent>>
    {
    ...
    auto SceneManager::getTransforms() -> Array<SmartPtr<ITransform>> &
    {
        return m_transforms;
    }

    auto SceneManager::getTransforms() const -> const Array<SmartPtr<ITransform>> &
    {
        return m_transforms;
    }
    ...
    auto SceneManager::getComponentFactoryIgnoreList() const -> Array<String>
    {
        RecursiveMutex::ScopedLock lock( m_mutex );
        return m_componentFactoryIgnoreList;
    }

    auto SceneManager::getComponentFactoryMap() const -> Map<String, String>
    {
        RecursiveMutex::ScopedLock lock( m_mutex );
    ...
    auto SceneManager::getComponentFactoryType( const String &type ) const -> String
    {
        RecursiveMutex::ScopedLock lock( m_mutex );
    ...
    auto SceneManager::getTransformState( u32 id, time_interval t, Transform3<real_Num> &transform )
        -> bool
    {
        ...
        const auto &times = m_transformTimes[id];
        const auto &transforms = m_transformStates[id];
        ...
        for( size_t i = 1; i < times.size(); ++i )
        {
            ...
            if( time < t )
            {
                ...
                if( diff > 0.0 )
                {
                    ...
                    auto position =
                        Math<real_Num>::lerp( position1, position0, static_cast<real_Num>( delta ) );

                    auto orientation = Quaternion<real_Num>::slerp( static_cast<real_Num>( delta ),
                                                                    orientation1, orientation0, true );
    ...
    auto SceneManager::getRegisteredComponents( Thread::UpdateState state, Thread::Task task ) const
        -> SharedPtr<Array<SmartPtr<IComponent>>>
    {
    ...
    auto SceneManager::getNumActors() const -> s32
    {
        return m_numActors;
    }
    ...
    fb::u32 *SceneManager::getFlagsPtr( u32 id )
    {
        return &m_actorCurrentFlags[id];
    }

    fb::u32 *SceneManager::getPreviousFlagsPtr( u32 id )
    {
        return &m_actorPreviousFlags[id];
    }

    fb::u32 *SceneManager::getNewFlagsPtr( u32 id )
    {
        return &m_actorNewFlags[id];
    }

    fb::u32 *SceneManager::getCurrentFlagsPtr( u32 id )
    {
        return &m_actorCurrentFlags[id];
    }
